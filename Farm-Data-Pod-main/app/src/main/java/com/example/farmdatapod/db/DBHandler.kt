...
//                + "FOREIGN KEY(" + hubUserIdCol + ") REFERENCES " + usersTableName + "(" + idCol + "))")
        ...
        val createOrganisationCustomerUserTable = ("CREATE TABLE " + ORGANISATION_CUSTOMER_USER_TABLE_NAME + " ("
                ...
                + ORGANISATION_CUSTOMER_USER_PHONE_NUMBER_COL + " INTEGER, "
                ...
                + ORGANISATION_CUSTOMER_USER_PHONE_NUMBER1_COL + " INTEGER, "
                ...
                + ORGANISATION_CUSTOMER_USER_PHONE_NUMBER2_COL + " INTEGER, "
                ...
                + ORGANISATION_CUSTOMER_USER_PHONE_NUMBER3_COL + " INTEGER, "
...
//        db.execSQL(createCigFarmerFieldRegistrationTable)
    ...
    fun insertUser(user: User): Long {
        ...
        values.put(idCol, user.id)  // Direct access to properties
        values.put(lastNameCol, user.last_name)
        values.put(otherNameCol, user.other_name)
        values.put(userTypeCol, user.user_type)
        values.put(emailCol, user.email)
        values.put(roleCol, user.role)
        values.put(emailVerifiedCol, user.email_verified)
        // values.put(emailVerifiedCol, if (user.isEmailVerified()) 1 else 0)
        values.put(passwordCol, user.password)
        values.put(verificationTokenCol, user.verification_token)
        values.put(createdAtCol, user.created_at)
        values.put(updatedAtCol, user.updated_at)

        val id = db.insert(usersTableName, null, values)
        db.close()
        return id
    }

    // Insert hub method
    fun insertHub(hub: Hub): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(regionCol, hub.region)
        values.put(hubNameCol, hub.hub_name)
        values.put(hubCodeCol, hub.hub_code)
        values.put(addressCol, hub.address)
        values.put(yearEstablishedCol, hub.year_established)
        values.put(ownershipCol, hub.ownership)
        values.put(floorSizeCol, hub.floor_size)
        values.put(facilitiesCol, hub.facilities)
        values.put(inputCenterCol, hub.input_center)
        values.put(typeOfBuildingCol, hub.type_of_building)
        values.put(longitudeCol, hub.longitude)
        values.put(latitudeCol, hub.latitude)
        values.put(hubUserIdCol, hub.user_id)
        values.put(IS_OFFLINE_COL, 1) // Mark as offline

        // Insert hub data
        val hubResult = db.insert(HUB_TABLE_NAME, null, values)
        if (hubResult == -1L) {
            return false
        }

        // Insert key contacts associated with this hub
        val keyContacts: List<KeyContact>? = hub.key_contacts
        if (keyContacts != null) {
            for (keyContact in keyContacts) {
                keyContact.hub_id = hubResult.toInt()
                val contactInserted = insertKeyContact(keyContact)
                if (!contactInserted) {
                    return false
                }
            }
    ...
    // Insert buying center method
    fun insertBuyingCenter(buyingCenter: BuyingCenter): Boolean {
        val db = this.writableDatabase
        ...
        try {
            val values = ContentValues().apply {
                put(HUB_COL, buyingCenter.hub)
                put(COUNTY_COL, buyingCenter.county)
                put(SUB_COUNTY_COL, buyingCenter.sub_county)
                put(WARD_COL, buyingCenter.ward)
                put(VILLAGE_COL, buyingCenter.village)
                put(BUYING_CENTER_NAME_COL, buyingCenter.buying_center_name)
                put(BUYING_CENTER_CODE_COL, buyingCenter.buying_center_code)
                put(BUYING_CENTER_ADDRESS_COL, buyingCenter.address)
                put(BUYING_CENTER_YEAR_ESTABLISHED_COL, buyingCenter.year_established)
                put(BUYING_CENTER_OWNERSHIP_COL, buyingCenter.ownership)
                put(BUYING_CENTER_FLOOR_SIZE_COL, buyingCenter.floor_size)
                put(BUYING_CENTER_FACILITIES_COL, buyingCenter.facilities)
                put(BUYING_CENTER_INPUT_CENTER_COL, buyingCenter.input_center)
                put(BUYING_CENTER_TYPE_OF_BUILDING_COL, buyingCenter.type_of_building)
                put(BUYING_CENTER_LOCATION_COL, buyingCenter.location)
                put(BUYING_CENTER_USER_ID_COL, buyingCenter.user_id)
                put(IS_OFFLINE_COL, 1) // Mark as offline
            }

            // Log the values being inserted
            Log.d("DBHandler", "Inserting BuyingCenter with values: $values")

            // Insert buying center data
            val buyingCenterResult = db.insert(BUYING_CENTER_TABLE_NAME, null, values)
            Log.d("DBHandler", "Insert result for BuyingCenter: $buyingCenterResult")

            if (buyingCenterResult == -1L) {
                // Failed to insert buying center
                db.endTransaction()
                return false
            }

            // Insert key contacts associated with this buying center
            val keyContacts: List<KeyContact>? = buyingCenter.key_contacts
            Log.d("DBHandler", "Inserting KeyContacts for BuyingCenter with ID: ${buyingCenterResult.toInt()}")

            if (keyContacts != null) {
                for (keyContact in keyContacts) {
                    keyContact.buying_center_id = buyingCenterResult.toInt()
                    Log.d("DBHandler", "Inserting KeyContact: $keyContact")
                    val contactInserted = insertKeyContact(keyContact)
                    if (!contactInserted) {
                        // Failed to insert a key contact
                        db.endTransaction()
                        return false
                    }
                }
            ...
            e.printStackTrace()
            Log.e("DBHandler", "Exception during database transaction: ${e.message}")
        } finally {
            db.endTransaction()
        }
    ...
    fun insertCIG(cig: CIG): Boolean {
        ...
        try {
            val values = ContentValues().apply {
                put(CIG_HUB_COL, cig.hub)
                put(CIG_NAME_COL, cig.cig_name)
                put(NO_OF_MEMBERS_COL, cig.no_of_members)
                put(DATE_ESTABLISHED_COL, cig.date_established)
                put(CONSTITUTION_COL, cig.constitution)
                put(REGISTRATION_COL, cig.registration)
                put(ELECTIONS_HELD_COL, cig.elections_held)
                put(DATE_OF_LAST_ELECTIONS_COL, cig.date_of_last_elections)
                put(MEETING_VENUE_COL, cig.meeting_venue)
                put(FREQUENCY_COL, cig.frequency)
                put(SCHEDULED_MEETING_DAY_COL, cig.scheduled_meeting_day)
                put(SCHEDULED_MEETING_TIME_COL, cig.scheduled_meeting_time)
                put(CIG_USER_ID_COL, cig.user_id)
                put(IS_OFFLINE_COL, 1) // Mark as offline
            }

            // Log the values being inserted
            Log.d("DBHandler", "Inserting CIG with values: $values")

            // Insert CIG data
            val cigResult = db.insert(CIG_TABLE_NAME, null, values)
            Log.d("DBHandler", "Insert result for CIG: $cigResult")

            if (cigResult == -1L) {
                // Failed to insert CIG
                db.endTransaction()
                return false
            }

            // Insert members associated with this CIG
            val members: List<Member>? = cig.members
            Log.d("DBHandler", "Inserting Members for CIG with ID: ${cigResult.toInt()}")

            if (members != null) {
                for (member in members) {
                    member.cig_id = cigResult.toInt()
                    Log.d("DBHandler", "Inserting Member: $member")
                    val memberInserted = insertMember(member)
                    if (!memberInserted) {
                        // Failed to insert a member
                        db.endTransaction()
                        return false
                    }
                }
    ...
    //Insert Processor Users method
    fun insertProcessorUser(processingUser: ProcessingUser): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(PROCESSING_USER_OTHER_NAME_COL, processingUser.other_name)
        values.put(PROCESSING_USER_LAST_NAME_COL, processingUser.last_name)
        values.put(PROCESSING_USER_PROCESSOR_CODE_COL, processingUser.processor_code)
        values.put(PROCESSING_USER_PROCESSING_PLANT_COL, processingUser.processing_plant)
        values.put(PROCESSING_USER_ID_NUMBER_COL, processingUser.id_number)
        values.put(PROCESSING_USER_GENDER_COL, processingUser.gender)
        values.put(PROCESSING_USER_DATE_OF_BIRTH_COL, processingUser.date_of_birth)
        values.put(PROCESSING_USER_EMAIL_COL, processingUser.email)
        values.put(PROCESSING_USER_PHONE_NUMBER_COL, processingUser.phone_number)
        values.put(PROCESSING_USER_EDUCATION_LEVEL_COL, processingUser.education_level)
        values.put(PROCESSING_USER_HUB_COL, processingUser.hub)
        values.put(PROCESSING_USER_BUYING_CENTER_COL, processingUser.buying_center)
        values.put(PROCESSING_USER_COUNTY_COL, processingUser.county)
        values.put(PROCESSING_USER_SUB_COUNTY_COL, processingUser.sub_county)
        values.put(PROCESSING_USER_WARD_COL, processingUser.ward)
        values.put(PROCESSING_USER_VILLAGE_COL, processingUser.village)
        values.put(IS_OFFLINE_COL, 1)
        values.put(PROCESSING_USER_USER_ID_COL, processingUser.user_id)

        val result = db.insert(PROCESSING_USER_TABLE_NAME, null, values)
        db.close()
        return result != -1L
    }
    ...
    //Insert Custom users method
    fun insertCustomUser(customUser: CustomUser): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put("other_name", customUser.other_name)
        values.put("last_name", customUser.last_name)
        values.put("staff_code", customUser.staff_code)
        values.put("id_number", customUser.id_number)
        values.put("gender", customUser.gender)
        values.put("date_of_birth", customUser.date_of_birth)
        values.put("email", customUser.email)
        values.put("phone_number", customUser.phone_number)
        values.put("education_level", customUser.education_level)
        values.put("role", customUser.role)
        values.put("reporting_to", customUser.reporting_to)
        values.put("is_offline", 1)
        values.put("user_id", customUser.user_id)

        val result = db.insert("CustomUsers", null, values)
        db.close()
        return result != -1L
    }
    ...
    // Insert Individual Customer User
    fun insertIndividualCustomer(customer: IndividualCustomer): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()


        values.put(INDIVIDUAL_CUSTOMER_USER_OTHER_NAME_COL, customer.other_name)
        values.put(INDIVIDUAL_CUSTOMER_USER_LAST_NAME_COL, customer.last_name)
        values.put(INDIVIDUAL_CUSTOMER_USER_CUSTOMER_CODE_COL, customer.customer_code)
        values.put(INDIVIDUAL_CUSTOMER_USER_ID_NUMBER_COL, customer.id_number)
        values.put(INDIVIDUAL_CUSTOMER_USER_GENDER_COL, customer.gender)
        values.put(INDIVIDUAL_CUSTOMER_USER_DATE_OF_BIRTH_COL, customer.date_of_birth)
        values.put(INDIVIDUAL_CUSTOMER_USER_EMAIL_COL, customer.email)
        values.put(INDIVIDUAL_CUSTOMER_USER_PHONE_NUMBER_COL, customer.phone_number)
        values.put(INDIVIDUAL_CUSTOMER_USER_COUNTY_COL, customer.county)
        values.put(INDIVIDUAL_CUSTOMER_USER_SUB_COUNTY_COL, customer.sub_county)
        values.put(INDIVIDUAL_CUSTOMER_USER_WARD_COL, customer.ward)
        values.put(INDIVIDUAL_CUSTOMER_USER_VILLAGE_COL, customer.village)
        values.put(INDIVIDUAL_CUSTOMER_USER_USER_AUTHORISED_COL, customer.user_authorised)
        values.put(INDIVIDUAL_CUSTOMER_USER_AUTHORISATION_TOKEN_COL, customer.authorisation_token)
        values.put(IS_OFFLINE_COL, 1)
        values.put(INDIVIDUAL_CUSTOMER_USER_USER_ID_COL, customer.user_id)

        // Insert individual customer data
        val customerResult = db.insert(INDIVIDUAL_CUSTOMER_USER_TABLE_NAME, null, values)
        if (customerResult == -1L) {
            return false
        }

        // Insert products associated with this customer
        val products: List<Product>? = customer.products
        if (products != null) {
            for (product in products) {
                product.individual_customer_id = customerResult.toInt()
                val productInserted = insertProduct(product)
                if (!productInserted) {
                    return false
                }
            }
    ...
    // Insert Organisational Customer User
    fun insertOrganisationalCustomer(customer: OrganisationalCustomer): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(ORGANISATION_CUSTOMER_USER_COMPANY_NAME_COL, customer.company_name)
        values.put(ORGANISATION_CUSTOMER_USER_CUSTOMER_CODE_COL, customer.customer_code)
        values.put(ORGANISATION_CUSTOMER_USER_REGISTRATION_NUMBER_COL, customer.registration_number)
        values.put(ORGANISATION_CUSTOMER_USER_SECTOR_COL, customer.sector)
        values.put(ORGANISATION_CUSTOMER_USER_DATE_OF_REGISTRATION_COL, customer.date_of_registration)
        values.put(ORGANISATION_CUSTOMER_USER_EMAIL_COL, customer.email)
        values.put(ORGANISATION_CUSTOMER_USER_PHONE_NUMBER_COL, customer.phone_number)
        values.put(ORGANISATION_CUSTOMER_USER_COUNTY_COL, customer.county)
        values.put(ORGANISATION_CUSTOMER_USER_SUB_COUNTY_COL, customer.sub_county)
        values.put(ORGANISATION_CUSTOMER_USER_WARD_COL, customer.ward)
        values.put(ORGANISATION_CUSTOMER_USER_VILLAGE_COL, customer.village)
        values.put(ORGANISATION_CUSTOMER_USER_OTHER_NAME1_COL, customer.other_name1)
        values.put(ORGANISATION_CUSTOMER_USER_LAST_NAME1_COL, customer.last_name1)
        values.put(ORGANISATION_CUSTOMER_USER_ID_NUMBER1_COL, customer.id_number1)
        values.put(ORGANISATION_CUSTOMER_USER_GENDER1_COL, customer.gender1)
        values.put(ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH1_COL, customer.date_of_birth1)
        values.put(ORGANISATION_CUSTOMER_USER_EMAIL1_COL, customer.email1)
        values.put(ORGANISATION_CUSTOMER_USER_PHONE_NUMBER1_COL, customer.phone_number1)
        values.put(ORGANISATION_CUSTOMER_USER_OTHER_NAME2_COL, customer.other_name2)
        values.put(ORGANISATION_CUSTOMER_USER_LAST_NAME2_COL, customer.last_name2)
        values.put(ORGANISATION_CUSTOMER_USER_ID_NUMBER2_COL, customer.id_number2)
        values.put(ORGANISATION_CUSTOMER_USER_GENDER2_COL, customer.gender2)
        values.put(ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH2_COL, customer.date_of_birth2)
        values.put(ORGANISATION_CUSTOMER_USER_EMAIL2_COL, customer.email2)
        values.put(ORGANISATION_CUSTOMER_USER_PHONE_NUMBER2_COL, customer.phone_number2)
        values.put(ORGANISATION_CUSTOMER_USER_OTHER_NAME3_COL, customer.other_name3)
        values.put(ORGANISATION_CUSTOMER_USER_LAST_NAME3_COL, customer.last_name3)
        values.put(ORGANISATION_CUSTOMER_USER_ID_NUMBER3_COL, customer.id_number3)
        values.put(ORGANISATION_CUSTOMER_USER_GENDER3_COL, customer.gender3)
        values.put(ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH3_COL, customer.date_of_birth3)
        values.put(ORGANISATION_CUSTOMER_USER_EMAIL3_COL, customer.email3)
        values.put(ORGANISATION_CUSTOMER_USER_PHONE_NUMBER3_COL, customer.phone_number3)
        values.put(ORGANISATION_CUSTOMER_USER_USER_AUTHORISED_COL, customer.user_authorised)
        values.put(ORGANISATION_CUSTOMER_USER_AUTHORISATION_TOKEN_COL, customer.authorisation_token)
        values.put(IS_OFFLINE_COL, 1) // Mark as offline
        values.put(ORGANISATION_CUSTOMER_USER_USER_ID_COL, customer.user_id)

        // Insert organizational customer data
        val customerResult = db.insert(ORGANISATION_CUSTOMER_USER_TABLE_NAME, null, values)
        if (customerResult == -1L) {
            return false
        }
        ...
        if (products != null) {
            for (product in products) {
                product.organisation_customer_id = customerResult.toInt()
                val productInserted = insertProduct(product)
                if (!productInserted) {
                    return false
                }
    ...
    fun insertProducerBiodata(producer: ProducerBiodata): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        // Insert Producer Biodata
        values.put(PRODUCER_OTHER_NAME_COL, producer.other_name)
        values.put(PRODUCER_LAST_NAME_COL, producer.last_name)
        values.put(PRODUCER_FARMER_CODE_COL, producer.farmer_code)
        values.put(PRODUCER_ID_NUMBER_COL, producer.id_number)
        values.put(PRODUCER_DATE_OF_BIRTH_COL, producer.date_of_birth)
        values.put(PRODUCER_EMAIL_COL, producer.email)
        values.put(PRODUCER_PHONE_NUMBER_COL, producer.phone_number)
        values.put(PRODUCER_HUB_COL, producer.hub)
        values.put(PRODUCER_BUYING_CENTER_COL, producer.buying_center)
        values.put(PRODUCER_GENDER_COL, producer.gender)
        values.put(PRODUCER_EDUCATION_LEVEL_COL, producer.education_level)
        values.put(PRODUCER_COUNTY_COL, producer.county)
        values.put(PRODUCER_SUB_COUNTY_COL, producer.sub_county)
        values.put(PRODUCER_WARD_COL, producer.ward)
        values.put(PRODUCER_VILLAGE_COL, producer.village)

        // Convert primary_producer list to JSON string
        val gson = Gson()
        val primaryProducerJson = gson.toJson(producer.primary_producer)
        values.put(PRODUCER_PRIMARY_PRODUCER_COL, primaryProducerJson)

        values.put(PRODUCER_TOTAL_LAND_SIZE_COL, producer.total_land_size)
        values.put(PRODUCER_CULTIVATE_LAND_SIZE_COL, producer.cultivate_land_size)
        values.put(PRODUCER_HOMESTEAD_SIZE_COL, producer.homestead_size)
        values.put(PRODUCER_UNCULTIVATED_LAND_SIZE_COL, producer.uncultivated_land_size)
        values.put(PRODUCER_FARM_ACCESSIBILITY_COL, producer.farm_accessibility)
        values.put(PRODUCER_NUMBER_OF_FAMILY_WORKERS_COL, producer.number_of_family_workers)
        values.put(PRODUCER_NUMBER_OF_HIRED_WORKERS_COL, producer.number_of_hired_workers)
        values.put(PRODUCER_ACCESS_TO_IRRIGATION_COL, producer.access_to_irrigation)
        values.put(PRODUCER_CROP_LIST_COL, producer.crop_list)
        values.put(PRODUCER_FARMER_INTEREST_IN_EXTENSION_COL, producer.farmer_interest_in_extension)
        values.put(PRODUCER_KNOWLEDGE_RELATED_COL, producer.knowledge_related)
        values.put(PRODUCER_SOIL_RELATED_COL, producer.soil_related)
        values.put(PRODUCER_COMPOST_RELATED_COL, producer.compost_related)
        values.put(PRODUCER_NUTRITION_RELATED_COL, producer.nutrition_related)
        values.put(PRODUCER_PESTS_RELATED_COL, producer.pests_related)
        values.put(PRODUCER_DISEASE_RELATED_COL, producer.disease_related)
        values.put(PRODUCER_QUALITY_RELATED_COL, producer.quality_related)
        values.put(PRODUCER_MARKET_RELATED_COL, producer.market_related)
        values.put(PRODUCER_FOOD_LOSS_RELATED_COL, producer.food_loss_related)
        values.put(PRODUCER_FINANCE_RELATED_COL, producer.finance_related)
        values.put(PRODUCER_WEATHER_RELATED_COL, producer.weather_related)
        values.put(PRODUCER_DAIRY_CATTLE_COL, producer.dairy_cattle)
        values.put(PRODUCER_BEEF_CATTLE_COL, producer.beef_cattle)
        values.put(PRODUCER_SHEEP_COL, producer.sheep)
        values.put(PRODUCER_POULTRY_COL, producer.poultry)
        values.put(PRODUCER_PIGS_COL, producer.pigs)
        values.put(PRODUCER_RABBITS_COL, producer.rabbits)
        values.put(PRODUCER_BEEHIVES_COL, producer.beehives)
        values.put(PRODUCER_DONKEYS_COL, producer.donkeys)
        values.put(PRODUCER_GOATS_COL, producer.goats)
        values.put(PRODUCER_CAMELS_COL, producer.camels)
        values.put(PRODUCER_AQUACULTURE_COL, producer.aquaculture)
        values.put(PRODUCER_HOUSING_TYPE_COL, producer.housing_type)
        values.put(PRODUCER_HOUSING_FLOOR_COL, producer.housing_floor)
        values.put(PRODUCER_HOUSING_ROOF_COL, producer.housing_roof)
        values.put(PRODUCER_LIGHTING_FUEL_COL, producer.lighting_fuel)
        values.put(PRODUCER_COOKING_FUEL_COL, producer.cooking_fuel)
        values.put(PRODUCER_WATER_FILTER_COL, producer.water_filter)
        values.put(PRODUCER_WATER_TANK_GREATER_THAN_5000LTS_COL, producer.water_tank_greater_than_5000lts)
        values.put(PRODUCER_HAND_WASHING_FACILITIES_COL, producer.hand_washing_facilities)
        values.put(PRODUCER_PPES_COL, producer.ppes)
        values.put(PRODUCER_WATER_WELL_OR_WEIR_COL, producer.water_well_or_weir)
        values.put(PRODUCER_IRRIGATION_PUMP_COL, producer.irrigation_pump)
        values.put(PRODUCER_HARVESTING_EQUIPMENT_COL, producer.harvesting_equipment)
        values.put(PRODUCER_TRANSPORTATION_TYPE_COL, producer.transportation_type)
        values.put(PRODUCER_TOILET_FLOOR_COL, producer.toilet_floor)
        values.put(PRODUCER_USER_APPROVED_COL, producer.user_approved)
        values.put(PRODUCER_TA_COL, producer.ta)
        values.put(PRODUCER_USER_ID_COL, producer.user_id)
        values.put(IS_OFFLINE_COL, 1)

        // Insert producer biodata
        val producerResult = db.insert(PRODUCER_BIODATA_TABLE_NAME, null, values)
        ...
        if (commercialProduces != null) {
            for (produce in commercialProduces) {
                produce.producer_biodata_id = producerResult.toInt()
                val produceInserted = insertCommercialProduce(produce)
                if (!produceInserted) {
                    return false
                }
            }
        ...
        if (domesticProduces != null) {
            for (produce in domesticProduces) {
                produce.producer_biodata_id = producerResult.toInt()
                val produceInserted = insertDomesticProduce(produce)
                if (!produceInserted) {
                    return false
                }
            }
    ...
    // Insert farmer field registration
//    fun insertFieldRegistration(fieldRegistration: FarmerFieldRegistration): Boolean {
//        val db = this.writableDatabase
//        val values = ContentValues()
//
//        // Insert Field Registration
//        values.put(FARMER_FIELD_PRODUCER_COL, fieldRegistration.producer)
//        values.put(FARMER_FIELD_FIELD_NUMBER_COL, fieldRegistration.field_number)
//        values.put(FARMER_FIELD_FIELD_SIZE_COL, fieldRegistration.field_size)
//        values.put(FARMER_FIELD_CROP1_COL, fieldRegistration.crop1)
//        values.put(FARMER_FIELD_CROP_VARIETY1_COL, fieldRegistration.crop_variety1)
//        values.put(FARMER_FIELD_DATE_PLANTED1_COL, fieldRegistration.date_planted1)
//        values.put(FARMER_FIELD_DATE_OF_HARVEST1_COL, fieldRegistration.date_of_harvest1)
//        values.put(FARMER_FIELD_POPULATION1_COL, fieldRegistration.population1)
//        values.put(FARMER_FIELD_BASELINE_YIELD_LAST_SEASON1_COL, fieldRegistration.baseline_yield_last_season1)
//        values.put(FARMER_FIELD_BASELINE_INCOME_LAST_SEASON1_COL, fieldRegistration.baseline_income_last_season1)
//        values.put(FARMER_FIELD_BASELINE_COST_OF_PRODUCTION_LAST_SEASON1_COL, fieldRegistration.baseline_cost_of_production_last_season1)
//        values.put(FARMER_FIELD_CROP2_COL, fieldRegistration.crop2)
//        values.put(FARMER_FIELD_CROP_VARIETY2_COL, fieldRegistration.crop_variety2)
//        values.put(FARMER_FIELD_DATE_PLANTED2_COL, fieldRegistration.date_planted2)
//        values.put(FARMER_FIELD_DATE_OF_HARVEST2_COL, fieldRegistration.date_of_harvest2)
//        values.put(FARMER_FIELD_POPULATION2_COL, fieldRegistration.population2)
//        values.put(FARMER_FIELD_BASELINE_YIELD_LAST_SEASON2_COL, fieldRegistration.baseline_yield_last_season2)
//        values.put(FARMER_FIELD_BASELINE_INCOME_LAST_SEASON2_COL, fieldRegistration.baseline_income_last_season2)
//        values.put(FARMER_FIELD_BASELINE_COST_OF_PRODUCTION_LAST_SEASON2_COL, fieldRegistration.baseline_cost_of_production_last_season2)
//        values.put(FARMER_FIELD_PRODUCER_BIODATA_ID_FK_COL, fieldRegistration.producer_biodata_id)
//        values.put(FARMER_FIELD_USER_ID_FK_COL, fieldRegistration.user_id)
//        values.put(IS_OFFLINE_COL, 1)
//
//        // Insert field registration
//        val result = db.insert(FARMER_FIELD_REGISTRATION_TABLE_NAME, null, values)
//        return result != -1L
//    }

//    // Insert CIG farmer field registration
//    fun insertCIGFieldRegistration(fieldRegistration: FarmerFieldRegistration): Boolean {
//        val db = this.writableDatabase
//        val values = ContentValues()
//
//        // Insert Field Registration
//        values.put(CIG_FARMER_FIELD_PRODUCER_COL, fieldRegistration.producer)
//        values.put(CIG_FARMER_FIELD_FIELD_NUMBER_COL, fieldRegistration.field_number)
//        values.put(CIG_FARMER_FIELD_FIELD_SIZE_COL, fieldRegistration.field_size)
//        values.put(CIG_FARMER_FIELD_CROP1_COL, fieldRegistration.crop1)
//        values.put(CIG_FARMER_FIELD_CROP_VARIETY1_COL, fieldRegistration.crop_variety1)
//        values.put(CIG_FARMER_FIELD_DATE_PLANTED1_COL, fieldRegistration.date_planted1)
//        values.put(CIG_FARMER_FIELD_DATE_OF_HARVEST1_COL, fieldRegistration.date_of_harvest1)
//        values.put(CIG_FARMER_FIELD_POPULATION1_COL, fieldRegistration.population1)
//        values.put(CIG_FARMER_FIELD_BASELINE_YIELD_LAST_SEASON1_COL, fieldRegistration.baseline_yield_last_season1)
//        values.put(CIG_FARMER_FIELD_BASELINE_INCOME_LAST_SEASON1_COL, fieldRegistration.baseline_income_last_season1)
//        values.put(CIG_FARMER_FIELD_BASELINE_COST_OF_PRODUCTION_LAST_SEASON1_COL, fieldRegistration.baseline_cost_of_production_last_season1)
//        values.put(CIG_FARMER_FIELD_CROP2_COL, fieldRegistration.crop2)
//        values.put(CIG_FARMER_FIELD_CROP_VARIETY2_COL, fieldRegistration.crop_variety2)
//        values.put(CIG_FARMER_FIELD_DATE_PLANTED2_COL, fieldRegistration.date_planted2)
//        values.put(CIG_FARMER_FIELD_DATE_OF_HARVEST2_COL, fieldRegistration.date_of_harvest2)
//        values.put(CIG_FARMER_FIELD_POPULATION2_COL, fieldRegistration.population2)
//        values.put(CIG_FARMER_FIELD_BASELINE_YIELD_LAST_SEASON2_COL, fieldRegistration.baseline_yield_last_season2)
//        values.put(CIG_FARMER_FIELD_BASELINE_INCOME_LAST_SEASON2_COL, fieldRegistration.baseline_income_last_season2)
//        values.put(CIG_FARMER_FIELD_BASELINE_COST_OF_PRODUCTION_LAST_SEASON2_COL, fieldRegistration.baseline_cost_of_production_last_season2)
//        values.put(CIG_FARMER_FIELD_PRODUCER_BIODATA_ID_FK_COL, fieldRegistration.producer_biodata_id)
//        values.put(CIG_FARMER_FIELD_USER_ID_FK_COL, fieldRegistration.user_id)
//        values.put(IS_OFFLINE_COL, 1)
//
//        // Insert field registration
//        val result = db.insert(CIG_FARMER_FIELD_REGISTRATION_TABLE_NAME, null, values)
//        return result != -1L
//    }

    // Insert Season Planning
    fun insertSeasonPlanning(seasonPlanning: SeasonPlanning): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        // Populate ContentValues for SeasonPlanning
        values.put(SEASON_PLANNING_PRODUCER_COL, seasonPlanning.producer)
        values.put(SEASON_PLANNING_FIELD_COL, seasonPlanning.fieldName)
        values.put(SEASON_PLANNING_PLANNED_DATE_OF_PLANTING_COL, seasonPlanning.planned_date_of_planting)
        values.put(SEASON_PLANNING_WEEK_NUMBER_COL, seasonPlanning.week_number)

        // JSON fields
        values.put(SEASON_PLANNING_NURSERY_COL, Gson().toJson(seasonPlanning.nursery))
        values.put(SEASON_PLANNING_SOIL_ANALYSIS_COL, Gson().toJson(seasonPlanning.soil_analysis))
        values.put(SEASON_PLANNING_LIMING_COL, Gson().toJson(seasonPlanning.liming))
        values.put(SEASON_PLANNING_TRANSPLANTING_COL, Gson().toJson(seasonPlanning.transplanting))
        values.put(SEASON_PLANNING_WEEDING_COL, Gson().toJson(seasonPlanning.weeding))
        values.put(SEASON_PLANNING_PRUNNING_THINNING_DESUCKERING_COL, Gson().toJson(seasonPlanning.prunning_thinning_desuckering))
        values.put(SEASON_PLANNING_MULCHING_COL, Gson().toJson(seasonPlanning.mulching))
        values.put(SEASON_PLANNING_GAPPING_COL, Gson().toJson(seasonPlanning.gapping))
        values.put(SEASON_PLANNING_HARVESTING_COL, Gson().toJson(seasonPlanning.harvesting))
        values.put(IS_OFFLINE_COL, 1) // Mark as offline

        // Insert SeasonPlanning and get the inserted ID
        val seasonPlanningId = db.insert(SEASON_PLANNING_TABLE_NAME, null, values)

        if (seasonPlanningId == -1L) {
            // Insertion failed
            return false
        }

        // Set the generated ID to the seasonPlanning object, casting Long to Int
        seasonPlanning.id = seasonPlanningId.toInt()

        // Insert related data using the proper seasonPlanningId
        seasonPlanning.marketProduces?.forEach { produce ->
            produce.season_planning_id = seasonPlanning.id
            if (!insertMarketProduce(produce)) return false
        }

        seasonPlanning.plan_nutritions?.forEach { nutrition ->
            nutrition.season_planning_id = seasonPlanning.id
            if (!insertPlanNutrition(nutrition)) return false
        }

        seasonPlanning.preventative_diseases?.forEach { disease ->
            disease.season_planning_id = seasonPlanning.id
            if (!insertPreventativeDisease(disease)) return false
        }

        seasonPlanning.preventative_pests?.forEach { pest ->
            pest.season_planning_id = seasonPlanning.id
            if (!insertPreventativePest(pest)) return false
        }

        seasonPlanning.plan_irrigations?.forEach { irrigation ->
            irrigation.season_planning_id = seasonPlanning.id
            if (!insertPlanIrrigation(irrigation)) return false
        }

        seasonPlanning.scouting_stations?.forEach { station ->
            station.season_planning_id = seasonPlanning.id
            if (!insertScoutingStation(station)) return false
        }
    ...
    // Insert buying and selling and quarantine
    fun insertBuyingFarmer(buyingFarmer: BuyingFarmer): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        try {
            values.put(BUYING_FARMER_BUYING_CENTER_COL, buyingFarmer.buying_center)
            values.put(BUYING_FARMER_PRODUCER_COL, buyingFarmer.producer)
            values.put(BUYING_FARMER_PRODUCE_COL, buyingFarmer.produce)
            values.put(BUYING_FARMER_GRN_NUMBER_COL, buyingFarmer.grn_number)
            values.put(BUYING_FARMER_UNIT_COL, buyingFarmer.unit)

            val qualityJson = Gson().toJson(buyingFarmer.quality)
            values.put(BUYING_FARMER_QUALITY_COL, qualityJson)
            Log.d("DBBuyingFragment", "Quality JSON to insert: $qualityJson")

            values.put(BUYING_FARMER_ACTION_COL, buyingFarmer.action)
            values.put(BUYING_FARMER_WEIGHT_COL, buyingFarmer.weight)
            values.put(BUYING_FARMER_LOADED_COL, buyingFarmer.loaded)
            values.put(BUYING_FARMER_USER_ID_FK_COL, buyingFarmer.user_id)
            values.put(IS_OFFLINE_COL, 1)

            val result = db.insert(BUYING_FARMER_TABLE_NAME, null, values)
            Log.d("DBBuyingFragment", "Insert result: $result")

            return result != -1L
        } catch (e: Exception) {
            Log.e("DBBuyingFragment", "Error inserting BuyingFarmer into database", e)
            return false
        } finally {
    ...
    fun insertQuarantine(quarantine: Quarantine): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(QUARANTINE_ACTION_COL, quarantine.action)
        values.put(QUARANTINE_APPROVED_BY_COL, quarantine.quarantine_approved_by)
        values.put(QUARANTINE_NEW_WEIGHT_IN_COL, quarantine.new_weight_in_after_sorting_or_regrading)
        values.put(QUARANTINE_NEW_WEIGHT_OUT_COL, quarantine.new_weight_out_after_sorting_or_regrading)
        values.put(QUARANTINE_BUYING_FARMER_ID_FK_COL, quarantine.buying_farmer_id)
        values.put(QUARANTINE_BUYING_CUSTOMER_ID_FK_COL, quarantine.buying_customer_id)
        values.put(IS_OFFLINE_COL, 1)

        val result = db.insert(QUARANTINE_TABLE_NAME, null, values)
        db.close()
        return result != -1L
    }
    ...
    // Insert make and recieve payments
    fun insertPaymentFarmer(paymentFarmer: PaymentFarmer): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(PAYMENT_FARMER_BUYING_CENTER_COL, paymentFarmer.buying_center)
        values.put(PAYMENT_FARMER_CIG_COL, paymentFarmer.cig)
        values.put(PAYMENT_FARMER_PRODUCER_COL, paymentFarmer.producer)
        values.put(PAYMENT_FARMER_GRN_COL, paymentFarmer.grn)
        values.put(PAYMENT_FARMER_NET_BALANCE_COL, paymentFarmer.net_balance)
        values.put(PAYMENT_FARMER_PAYMENT_TYPE_COL, paymentFarmer.payment_type)
        values.put(PAYMENT_FARMER_OUTSTANDING_LOAN_AMOUNT_COL, paymentFarmer.outstanding_loan_amount)
        values.put(PAYMENT_FARMER_PAYMENT_DUE_COL, paymentFarmer.payment_due)
        values.put(PAYMENT_FARMER_SET_LOAN_DEDUCTION_COL, paymentFarmer.set_loan_deduction)
        values.put(PAYMENT_FARMER_NET_BALANCE_BEFORE_COL, paymentFarmer.net_balance_before)
        values.put(PAYMENT_FARMER_NET_BALANCE_AFTER_LOAN_DEDUCTION_COL, paymentFarmer.net_balance_after_loan_deduction)
        values.put(PAYMENT_FARMER_COMMENT_COL, paymentFarmer.comment)
        values.put(PAYMENT_FARMER_USER_ID_FK_COL, paymentFarmer.user_id)
        values.put(IS_OFFLINE_COL, 1)

        val result = db.insert(PAYMENT_FARMER_TABLE_NAME, null, values)

        return result != -1L
    }

    fun insertPaymentCustomer(paymentCustomer: PaymentCustomer): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(PAYMENT_CUSTOMER_VILLAGE_OR_ESTATE_COL, paymentCustomer.village_or_estate)
        values.put(PAYMENT_CUSTOMER_CUSTOMER_COL, paymentCustomer.customer)
        values.put(PAYMENT_CUSTOMER_GRN_COL, paymentCustomer.grn)
        values.put(PAYMENT_CUSTOMER_AMOUNT_COL, paymentCustomer.amount)
        values.put(PAYMENT_CUSTOMER_NET_BALANCE_COL, paymentCustomer.net_balance)
        values.put(PAYMENT_CUSTOMER_PAYMENT_TYPE_COL, paymentCustomer.payment_type)
        values.put(PAYMENT_CUSTOMER_ENTER_AMOUNT_COL, paymentCustomer.enter_amount)
        values.put(PAYMENT_CUSTOMER_NET_BALANCE_BEFORE_COL, paymentCustomer.net_balance_before)
        values.put(PAYMENT_CUSTOMER_NET_BALANCE_AFTER_COL, paymentCustomer.net_balance_after)
        values.put(PAYMENT_CUSTOMER_COMMENT_COL, paymentCustomer.comment)
        values.put(PAYMENT_CUSTOMER_USER_ID_FK_COL, paymentCustomer.user_id)
        values.put(IS_OFFLINE_COL, 1) // Mark as offline

        val result = db.insert(PAYMENT_CUSTOMER_TABLE_NAME, null, values)
        return result != -1L
    }
    ...
    fun insertDispatchInput(dispatchInput: DispatchInput): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(DISPATCH_INPUT_GRN_COL, dispatchInput.grn)
        values.put(DISPATCH_INPUT_INPUT_COL, dispatchInput.input)
        values.put(DISPATCH_INPUT_DESCRIPTION_COL, dispatchInput.description)
        values.put(DISPATCH_INPUT_NUMBER_OF_UNITS_COL, dispatchInput.number_of_units)
        values.put(DISPATCH_INPUT_PLAN_JOURNEY_ID_FK_COL, dispatchInput.plan_journey_id)
        values.put(IS_OFFLINE_COL, 1)

        val result = db.insert(DISPATCH_INPUT_TABLE_NAME, null, values)
        return result != -1L
    }


    // Insert loading
    fun insertLoading(loading: Loading): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(LOADING_GRN_COL, loading.grn)
        values.put(LOADING_TOTAL_WEIGHT_COL, loading.total_weight)
        values.put(LOADING_TRUCK_LOADING_NUMBER_COL, loading.truck_loading_number)
        values.put(LOADING_FROM_COL, loading.from_)
        values.put("`$LOADING_TO_COL`", loading.to)
        values.put(LOADING_COMMENT_COL, loading.comment)
        values.put(LOADING_OFFLOADED_COL,  0)
        values.put(LOADING_USER_ID_FK_COL, loading.user_id)
        values.put(IS_OFFLINE_COL, 1)
        Log.d("InsertLoading", "Loading ContentValues: $values")

        val result = db.insert(LOADING_TABLE_NAME, null, values)
        db.close()
        return result != -1L
    }
    ...
    // Insert offloading
    fun insertOffloading(offloading: Offloading): Boolean {
        val db = this.writableDatabase
        val values = ContentValues()

        values.put(OFFLOADING_OFFLOADED_LOAD_COL, offloading.offloaded_load)
        values.put(OFFLOADING_TOTAL_WEIGHT_COL, offloading.total_weight)
        values.put(OFFLOADING_TRUCK_OFFLOADING_NUMBER_COL, offloading.truck_offloading_number)
        values.put(OFFLOADING_COMMENT_COL, offloading.comment)
        values.put(OFFLOADING_USER_ID_FK_COL, offloading.user_id)
        values.put(IS_OFFLINE_COL, 1)

        val result = db.insert(OFFLOADING_TABLE_NAME, null, values)
        db.close()
        return result != -1L
    }
    ...
    // Getting produces for customer price distributions


    private fun getProducerIdsByBuyingCenterCPD(buyingCenter: String): List<String> {
        val producerIds = mutableListOf<String>()
        val db = this.readableDatabase
        // Ensure to fetch the correct column (ID) that matches the producer field in extensionServices
        val query = "SELECT $PRODUCER_BIODATA_ID_COL FROM $PRODUCER_BIODATA_TABLE_NAME WHERE $PRODUCER_BUYING_CENTER_COL = ?"
    ...
    fun getAvailableMarketProduce(): List<MarketProduce> {
        val marketProduceList = mutableListOf<MarketProduce>()
        val db = this.readableDatabase

        // SQL query to get market produces with prices set, linked to extension services, and not sold
        val query = """
        SELECT mp.*
        FROM $MARKET_PRODUCE_TABLE_NAME mp
        JOIN $CUSTOMER_PRICE_DISTRIBUTION_TABLE_NAME cpd 
            ON mp.$MARKET_PRODUCE_ID_COL = cpd.$CUSTOMER_PRICE_DISTRIBUTION_PRODUCE_ID_FK_COL
        WHERE mp.$MARKET_PRODUCE_EXTENSION_SERVICE_ID_FK_COL IS NOT NULL
            AND mp.$MARKET_PRODUCE_SEASON_PLANNING_ID_FK_COL IS NULL
            AND cpd.$CUSTOMER_PRICE_DISTRIBUTION_SOLD_COL = 0
    """
        ...
        if (cursor.moveToFirst()) {
            do {
                val marketProduce = MarketProduce().apply {
                    ...
                    season_planning_id = cursor.getInt(cursor.getColumnIndexOrThrow(MARKET_PRODUCE_SEASON_PLANNING_ID_FK_COL)).takeIf { it != 0 }
                    extension_service_id = cursor.getInt(cursor.getColumnIndexOrThrow(MARKET_PRODUCE_EXTENSION_SERVICE_ID_FK_COL)).takeIf { it != 0 }
                }
    ...
    // Get cars when planning logistics
    @SuppressLint("Range")
    fun getPlanJourneyCars(): List<Car> {
        val carList: MutableList<Car> = ArrayList()
        val db = this.readableDatabase
        val columns = arrayOf(
            CAR_ID_COL, CAR_BODY_TYPE_COL, CAR_MODEL_COL, CAR_NUMBER_PLATE_COL,
            CAR_DRIVER1_NAME_COL, CAR_DRIVER2_NAME_COL
        )
        val cursor = db.query(CAR_TABLE_NAME, columns, null, null, null, null, null)
        if (cursor != null && cursor.moveToFirst()) {
            do {
                val car = Car().apply {
    ...
    @get:SuppressLint("Range")
    val offlineProcessingUsers: List<ProcessingUser>
        get() {
            val processingUsers: MutableList<ProcessingUser> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                PROCESSING_USER_ID_COL,
                PROCESSING_USER_OTHER_NAME_COL,
                PROCESSING_USER_LAST_NAME_COL,
                PROCESSING_USER_PROCESSOR_CODE_COL,
                PROCESSING_USER_PROCESSING_PLANT_COL,
                PROCESSING_USER_ID_NUMBER_COL,
                PROCESSING_USER_GENDER_COL,
                PROCESSING_USER_DATE_OF_BIRTH_COL,
                PROCESSING_USER_EMAIL_COL,
                PROCESSING_USER_PHONE_NUMBER_COL,
                PROCESSING_USER_EDUCATION_LEVEL_COL,
                PROCESSING_USER_HUB_COL,
                PROCESSING_USER_BUYING_CENTER_COL,
                PROCESSING_USER_COUNTY_COL,
                PROCESSING_USER_SUB_COUNTY_COL,
                PROCESSING_USER_WARD_COL,
                PROCESSING_USER_VILLAGE_COL,
                PROCESSING_USER_USER_ID_COL
            )
            val selection = IS_OFFLINE_COL + "=?"
            val selectionArgs = arrayOf("1") // 1 indicates true for is_offline

            val cursor = db.query(PROCESSING_USER_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val processingUser = ProcessingUser().apply {
                        id = cursor.getInt(cursor.getColumnIndex(PROCESSING_USER_ID_COL))
                        other_name = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_OTHER_NAME_COL))
                        last_name = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_LAST_NAME_COL))
                        processor_code = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_PROCESSOR_CODE_COL))
                        processing_plant = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_PROCESSING_PLANT_COL))
                        id_number = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_ID_NUMBER_COL))
                        gender = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_GENDER_COL))
                        date_of_birth = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_DATE_OF_BIRTH_COL))
                        email = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_EMAIL_COL))
                        phone_number = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_PHONE_NUMBER_COL))
                        education_level = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_EDUCATION_LEVEL_COL))
                        hub = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_HUB_COL))
                        buying_center = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_BUYING_CENTER_COL))
                        county = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_COUNTY_COL))
                        sub_county = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_SUB_COUNTY_COL))
                        ward = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_WARD_COL))
                        village = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_VILLAGE_COL))
                        user_id = cursor.getString(cursor.getColumnIndex(PROCESSING_USER_USER_ID_COL))
                    }

                    // Add Processing user to list
                    processingUsers.add(processingUser)
                } while (cursor.moveToNext())
    ...
    @get:SuppressLint("Range")
    val offlineCustomUsers: List<CustomUser>
        get() {
            val customUsers: MutableList<CustomUser> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                CUSTOM_USER_ID_COL,
                CUSTOM_USER_OTHER_NAME_COL,
                CUSTOM_USER_LAST_NAME_COL,
                CUSTOM_USER_STAFF_CODE_COL,
                CUSTOM_USER_ID_NUMBER_COL,
                CUSTOM_USER_GENDER_COL,
                CUSTOM_USER_DATE_OF_BIRTH_COL,
                CUSTOM_USER_EMAIL_COL,
                CUSTOM_USER_PHONE_NUMBER_COL,
                CUSTOM_USER_EDUCATION_LEVEL_COL,
                CUSTOM_USER_ROLE_COL,
                CUSTOM_USER_REPORTING_TO_COL,
                CUSTOM_USER_USER_ID_COL
            )
            val selection = IS_OFFLINE_COL + "=?"
            val selectionArgs = arrayOf("1") // 1 indicates true for is_offline

            val cursor = db.query(CUSTOM_USER_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val customUser = CustomUser().apply {
                        id = cursor.getInt(cursor.getColumnIndex(CUSTOM_USER_ID_COL))
                        other_name = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_OTHER_NAME_COL))
                        last_name = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_LAST_NAME_COL))
                        staff_code = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_STAFF_CODE_COL))
                        id_number = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_ID_NUMBER_COL))
                        gender = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_GENDER_COL))
                        date_of_birth = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_DATE_OF_BIRTH_COL))
                        email = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_EMAIL_COL))
                        phone_number = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_PHONE_NUMBER_COL))
                        education_level = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_EDUCATION_LEVEL_COL))
                        role = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_ROLE_COL))
                        reporting_to = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_REPORTING_TO_COL))
                        user_id = cursor.getString(cursor.getColumnIndex(CUSTOM_USER_USER_ID_COL))
                    }

                    // Add CustomUser to list
    ...
    @get:SuppressLint("Range")
    val offlineIndividualCustomers: List<IndividualCustomer>
        get() {
            val customerList: MutableList<IndividualCustomer> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                INDIVIDUAL_CUSTOMER_USER_ID_COL,
                INDIVIDUAL_CUSTOMER_USER_OTHER_NAME_COL,
                INDIVIDUAL_CUSTOMER_USER_LAST_NAME_COL,
                INDIVIDUAL_CUSTOMER_USER_CUSTOMER_CODE_COL,
                INDIVIDUAL_CUSTOMER_USER_ID_NUMBER_COL,
                INDIVIDUAL_CUSTOMER_USER_GENDER_COL,
                INDIVIDUAL_CUSTOMER_USER_DATE_OF_BIRTH_COL,
                INDIVIDUAL_CUSTOMER_USER_EMAIL_COL,
                INDIVIDUAL_CUSTOMER_USER_PHONE_NUMBER_COL,
                INDIVIDUAL_CUSTOMER_USER_COUNTY_COL,
                INDIVIDUAL_CUSTOMER_USER_SUB_COUNTY_COL,
                INDIVIDUAL_CUSTOMER_USER_WARD_COL,
                INDIVIDUAL_CUSTOMER_USER_VILLAGE_COL,
                INDIVIDUAL_CUSTOMER_USER_USER_AUTHORISED_COL,
                INDIVIDUAL_CUSTOMER_USER_AUTHORISATION_TOKEN_COL,
                IS_OFFLINE_COL,
                INDIVIDUAL_CUSTOMER_USER_USER_ID_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1") // 1 indicates true for is_offline

            val cursor = db.query(INDIVIDUAL_CUSTOMER_USER_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val customer = IndividualCustomer().apply {
                        id = cursor.getInt(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_ID_COL))
                        other_name = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_OTHER_NAME_COL))
                        last_name = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_LAST_NAME_COL))
                        customer_code = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_CUSTOMER_CODE_COL))
                        id_number = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_ID_NUMBER_COL))
                        gender = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_GENDER_COL))
                        date_of_birth = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_DATE_OF_BIRTH_COL))
                        email = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_EMAIL_COL))
                        phone_number = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_PHONE_NUMBER_COL))
                        county = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_COUNTY_COL))
                        sub_county = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_SUB_COUNTY_COL))
                        ward = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_WARD_COL))
                        village = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_VILLAGE_COL))
                        user_authorised = cursor.getInt(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_USER_AUTHORISED_COL)) == 1
                        authorisation_token = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_AUTHORISATION_TOKEN_COL))
                        user_id = cursor.getString(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_USER_USER_ID_COL))
                    }

                    // Get associated products
                    val products: List<Product> = getProductsForIndividualCustomer(customer.id)
    ...
    @SuppressLint("Range")
    fun getProductsForIndividualCustomer(customerId: Int): List<Product> {
        val productList: MutableList<Product> = ArrayList()
        val db = this.readableDatabase
        val columns = arrayOf(
            PRODUCT_ID_COL,
            PRODUCT_CATEGORY_COL,
            PRODUCT_PRODUCTS_INTERESTED_IN_COL,
            PRODUCT_VOLUME_IN_KGS_COL,
            PRODUCT_PACKAGING_COL,
            PRODUCT_QUALITY_COL,
            PRODUCT_FREQUENCY_COL,
            IS_OFFLINE_COL,
            INDIVIDUAL_CUSTOMER_ID_FK_COL,
            ORGANISATION_CUSTOMER_ID_FK_COL
        )
        val selection = "$INDIVIDUAL_CUSTOMER_ID_FK_COL=?"
        val selectionArgs = arrayOf(customerId.toString())

        val cursor = db.query(PRODUCT_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
        if (cursor != null && cursor.moveToFirst()) {
            do {
                val product = Product().apply {
                    id = cursor.getInt(cursor.getColumnIndex(PRODUCT_ID_COL))
                    category = cursor.getString(cursor.getColumnIndex(PRODUCT_CATEGORY_COL))
                    products_interested_in = cursor.getString(cursor.getColumnIndex(PRODUCT_PRODUCTS_INTERESTED_IN_COL))
                    volume_in_kgs = cursor.getString(cursor.getColumnIndex(PRODUCT_VOLUME_IN_KGS_COL))
                    packaging = cursor.getString(cursor.getColumnIndex(PRODUCT_PACKAGING_COL))
                    quality = cursor.getString(cursor.getColumnIndex(PRODUCT_QUALITY_COL))
                    frequency = cursor.getString(cursor.getColumnIndex(PRODUCT_FREQUENCY_COL))
                    individual_customer_id = cursor.getInt(cursor.getColumnIndex(INDIVIDUAL_CUSTOMER_ID_FK_COL))
                    organisation_customer_id = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_ID_FK_COL))
                }
    ...
    @get:SuppressLint("Range")
    val offlineOrganisationalCustomers: List<OrganisationalCustomer>
        get() {
            val customerList: MutableList<OrganisationalCustomer> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                ORGANISATION_CUSTOMER_USER_ID_COL,
                ORGANISATION_CUSTOMER_USER_COMPANY_NAME_COL,
                ORGANISATION_CUSTOMER_USER_CUSTOMER_CODE_COL,
                ORGANISATION_CUSTOMER_USER_REGISTRATION_NUMBER_COL,
                ORGANISATION_CUSTOMER_USER_SECTOR_COL,
                ORGANISATION_CUSTOMER_USER_DATE_OF_REGISTRATION_COL,
                ORGANISATION_CUSTOMER_USER_EMAIL_COL,
                ORGANISATION_CUSTOMER_USER_PHONE_NUMBER_COL,
                ORGANISATION_CUSTOMER_USER_COUNTY_COL,
                ORGANISATION_CUSTOMER_USER_SUB_COUNTY_COL,
                ORGANISATION_CUSTOMER_USER_WARD_COL,
                ORGANISATION_CUSTOMER_USER_VILLAGE_COL,
                ORGANISATION_CUSTOMER_USER_OTHER_NAME1_COL,
                ORGANISATION_CUSTOMER_USER_LAST_NAME1_COL,
                ORGANISATION_CUSTOMER_USER_ID_NUMBER1_COL,
                ORGANISATION_CUSTOMER_USER_GENDER1_COL,
                ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH1_COL,
                ORGANISATION_CUSTOMER_USER_EMAIL1_COL,
                ORGANISATION_CUSTOMER_USER_PHONE_NUMBER1_COL,
                ORGANISATION_CUSTOMER_USER_OTHER_NAME2_COL,
                ORGANISATION_CUSTOMER_USER_LAST_NAME2_COL,
                ORGANISATION_CUSTOMER_USER_ID_NUMBER2_COL,
                ORGANISATION_CUSTOMER_USER_GENDER2_COL,
                ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH2_COL,
                ORGANISATION_CUSTOMER_USER_EMAIL2_COL,
                ORGANISATION_CUSTOMER_USER_PHONE_NUMBER2_COL,
                ORGANISATION_CUSTOMER_USER_OTHER_NAME3_COL,
                ORGANISATION_CUSTOMER_USER_LAST_NAME3_COL,
                ORGANISATION_CUSTOMER_USER_ID_NUMBER3_COL,
                ORGANISATION_CUSTOMER_USER_GENDER3_COL,
                ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH3_COL,
                ORGANISATION_CUSTOMER_USER_EMAIL3_COL,
                ORGANISATION_CUSTOMER_USER_PHONE_NUMBER3_COL,
                ORGANISATION_CUSTOMER_USER_USER_AUTHORISED_COL,
                ORGANISATION_CUSTOMER_USER_AUTHORISATION_TOKEN_COL,
                IS_OFFLINE_COL,
                ORGANISATION_CUSTOMER_USER_USER_ID_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1")

            val cursor = db.query(ORGANISATION_CUSTOMER_USER_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val customer = OrganisationalCustomer().apply {
                        id = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_ID_COL))
                        company_name = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_COMPANY_NAME_COL))
                        customer_code = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_CUSTOMER_CODE_COL))
                        registration_number = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_REGISTRATION_NUMBER_COL))
                        sector = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_SECTOR_COL))
                        date_of_registration = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_DATE_OF_REGISTRATION_COL))
                        email = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_EMAIL_COL))
                        phone_number = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_PHONE_NUMBER_COL))
                        county = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_COUNTY_COL))
                        sub_county = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_SUB_COUNTY_COL))
                        ward = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_WARD_COL))
                        village = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_VILLAGE_COL))
                        other_name1 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_OTHER_NAME1_COL))
                        last_name1 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_LAST_NAME1_COL))
                        id_number1 = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_ID_NUMBER1_COL))
                        gender1 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_GENDER1_COL))
                        date_of_birth1 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH1_COL))
                        email1 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_EMAIL1_COL))
                        phone_number1 = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_PHONE_NUMBER1_COL))
                        other_name2 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_OTHER_NAME2_COL))
                        last_name2 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_LAST_NAME2_COL))
                        id_number2 = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_ID_NUMBER2_COL))
                        gender2 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_GENDER2_COL))
                        date_of_birth2 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH2_COL))
                        email2 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_EMAIL2_COL))
                        phone_number2 = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_PHONE_NUMBER2_COL))
                        other_name3 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_OTHER_NAME3_COL))
                        last_name3 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_LAST_NAME3_COL))
                        id_number3 = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_ID_NUMBER3_COL))
                        gender3 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_GENDER3_COL))
                        date_of_birth3 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_DATE_OF_BIRTH3_COL))
                        email3 = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_EMAIL3_COL))
                        phone_number3 = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_PHONE_NUMBER3_COL))
                        user_authorised = cursor.getInt(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_USER_AUTHORISED_COL)) == 1
                        authorisation_token = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_AUTHORISATION_TOKEN_COL))
                        user_id = cursor.getString(cursor.getColumnIndex(ORGANISATION_CUSTOMER_USER_USER_ID_COL))
                    }

                    // Get associated products
                    val products: List<Product> = getProductsForOrganisationalCustomer(customer.id)
                    customer.products = products
    ...
    @SuppressLint("Range")
    fun getProductsForOrganisationalCustomer(customerId: Int): List<Product> {
        val productList: MutableList<Product> = ArrayList()
        val db = this.readableDatabase
        val columns = arrayOf(
            PRODUCT_ID_COL,
        ...
        val selection = "$ORGANISATION_CUSTOMER_ID_FK_COL=?"
        val selectionArgs = arrayOf(customerId.toString())
    ...
    @get:SuppressLint("Range")
    val offlineProducerBiodata: List<ProducerBiodata>
        get() {
            val producerList: MutableList<ProducerBiodata> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                PRODUCER_BIODATA_ID_COL,
                PRODUCER_OTHER_NAME_COL,
                PRODUCER_LAST_NAME_COL,
                PRODUCER_FARMER_CODE_COL,
                PRODUCER_ID_NUMBER_COL,
                PRODUCER_GENDER_COL,
                PRODUCER_DATE_OF_BIRTH_COL,
                PRODUCER_EMAIL_COL,
                PRODUCER_PHONE_NUMBER_COL,
                PRODUCER_HUB_COL,
                PRODUCER_BUYING_CENTER_COL,
                PRODUCER_EDUCATION_LEVEL_COL,
                PRODUCER_COUNTY_COL,
                PRODUCER_SUB_COUNTY_COL,
                PRODUCER_WARD_COL,
                PRODUCER_VILLAGE_COL,
                PRODUCER_PRIMARY_PRODUCER_COL,
                PRODUCER_TOTAL_LAND_SIZE_COL,
                PRODUCER_CULTIVATE_LAND_SIZE_COL,
                PRODUCER_HOMESTEAD_SIZE_COL,
                PRODUCER_UNCULTIVATED_LAND_SIZE_COL,
                PRODUCER_FARM_ACCESSIBILITY_COL,
                PRODUCER_NUMBER_OF_FAMILY_WORKERS_COL,
                PRODUCER_NUMBER_OF_HIRED_WORKERS_COL,
                PRODUCER_ACCESS_TO_IRRIGATION_COL,
                PRODUCER_CROP_LIST_COL,
                PRODUCER_FARMER_INTEREST_IN_EXTENSION_COL,
                PRODUCER_KNOWLEDGE_RELATED_COL,
                PRODUCER_SOIL_RELATED_COL,
                PRODUCER_COMPOST_RELATED_COL,
                PRODUCER_NUTRITION_RELATED_COL,
                PRODUCER_PESTS_RELATED_COL,
                PRODUCER_DISEASE_RELATED_COL,
                PRODUCER_QUALITY_RELATED_COL,
                PRODUCER_MARKET_RELATED_COL,
                PRODUCER_FOOD_LOSS_RELATED_COL,
                PRODUCER_FINANCE_RELATED_COL,
                PRODUCER_WEATHER_RELATED_COL,
                PRODUCER_DAIRY_CATTLE_COL,
                PRODUCER_BEEF_CATTLE_COL,
                PRODUCER_SHEEP_COL,
                PRODUCER_POULTRY_COL,
                PRODUCER_PIGS_COL,
                PRODUCER_RABBITS_COL,
                PRODUCER_BEEHIVES_COL,
                PRODUCER_DONKEYS_COL,
                PRODUCER_GOATS_COL,
                PRODUCER_CAMELS_COL,
                PRODUCER_AQUACULTURE_COL,
                PRODUCER_HOUSING_TYPE_COL,
                PRODUCER_HOUSING_FLOOR_COL,
                PRODUCER_HOUSING_ROOF_COL,
                PRODUCER_LIGHTING_FUEL_COL,
                PRODUCER_COOKING_FUEL_COL,
                PRODUCER_WATER_FILTER_COL,
                PRODUCER_WATER_TANK_GREATER_THAN_5000LTS_COL,
                PRODUCER_HAND_WASHING_FACILITIES_COL,
                PRODUCER_PPES_COL,
                PRODUCER_WATER_WELL_OR_WEIR_COL,
                PRODUCER_IRRIGATION_PUMP_COL,
                PRODUCER_HARVESTING_EQUIPMENT_COL,
                PRODUCER_TRANSPORTATION_TYPE_COL,
                PRODUCER_TOILET_FLOOR_COL,
                PRODUCER_USER_APPROVED_COL,
                PRODUCER_TA_COL,
                PRODUCER_USER_ID_COL,
                IS_OFFLINE_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1")

            val cursor = db.query(PRODUCER_BIODATA_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val producer = ProducerBiodata().apply {
                        val primaryProducerString = cursor.getString(cursor.getColumnIndex(PRODUCER_PRIMARY_PRODUCER_COL))

                        id = cursor.getInt(cursor.getColumnIndex(PRODUCER_BIODATA_ID_COL))
                        other_name = cursor.getString(cursor.getColumnIndex(PRODUCER_OTHER_NAME_COL))
                        last_name = cursor.getString(cursor.getColumnIndex(PRODUCER_LAST_NAME_COL))
                        farmer_code = cursor.getString(cursor.getColumnIndex(PRODUCER_FARMER_CODE_COL))
                        id_number = cursor.getString(cursor.getColumnIndex(PRODUCER_ID_NUMBER_COL))
                        gender = cursor.getString(cursor.getColumnIndex(PRODUCER_GENDER_COL))
                        date_of_birth = cursor.getString(cursor.getColumnIndex(PRODUCER_DATE_OF_BIRTH_COL))
                        email = cursor.getString(cursor.getColumnIndex(PRODUCER_EMAIL_COL))
                        phone_number = cursor.getString(cursor.getColumnIndex(PRODUCER_PHONE_NUMBER_COL))
                        hub = cursor.getString(cursor.getColumnIndex(PRODUCER_HUB_COL))
                        buying_center = cursor.getString(cursor.getColumnIndex(PRODUCER_BUYING_CENTER_COL))
                        education_level = cursor.getString(cursor.getColumnIndex(PRODUCER_EDUCATION_LEVEL_COL))
                        county = cursor.getString(cursor.getColumnIndex(PRODUCER_COUNTY_COL))
                        sub_county = cursor.getString(cursor.getColumnIndex(PRODUCER_SUB_COUNTY_COL))
                        ward = cursor.getString(cursor.getColumnIndex(PRODUCER_WARD_COL))
                        village = cursor.getString(cursor.getColumnIndex(PRODUCER_VILLAGE_COL))

                        primary_producer = primaryProducerString?.split(",")?.map {
                            PrimaryProducer().apply {
                                response = it.trim()
                            }
                        }

                        total_land_size = cursor.getString(cursor.getColumnIndex(PRODUCER_TOTAL_LAND_SIZE_COL))
                        cultivate_land_size = cursor.getString(cursor.getColumnIndex(PRODUCER_CULTIVATE_LAND_SIZE_COL))
                        homestead_size = cursor.getString(cursor.getColumnIndex(PRODUCER_HOMESTEAD_SIZE_COL))
                        uncultivated_land_size = cursor.getString(cursor.getColumnIndex(PRODUCER_UNCULTIVATED_LAND_SIZE_COL))
                        farm_accessibility = cursor.getString(cursor.getColumnIndex(PRODUCER_FARM_ACCESSIBILITY_COL))
                        number_of_family_workers = cursor.getString(cursor.getColumnIndex(PRODUCER_NUMBER_OF_FAMILY_WORKERS_COL))
                        number_of_hired_workers = cursor.getString(cursor.getColumnIndex(PRODUCER_NUMBER_OF_HIRED_WORKERS_COL))
                        access_to_irrigation = cursor.getString(cursor.getColumnIndex(PRODUCER_ACCESS_TO_IRRIGATION_COL))
                        crop_list = cursor.getString(cursor.getColumnIndex(PRODUCER_CROP_LIST_COL))
                        farmer_interest_in_extension = cursor.getString(cursor.getColumnIndex(PRODUCER_FARMER_INTEREST_IN_EXTENSION_COL))
                        knowledge_related = cursor.getString(cursor.getColumnIndex(PRODUCER_KNOWLEDGE_RELATED_COL))
                        soil_related = cursor.getString(cursor.getColumnIndex(PRODUCER_SOIL_RELATED_COL))
                        compost_related = cursor.getString(cursor.getColumnIndex(PRODUCER_COMPOST_RELATED_COL))
                        nutrition_related = cursor.getString(cursor.getColumnIndex(PRODUCER_NUTRITION_RELATED_COL))
                        pests_related = cursor.getString(cursor.getColumnIndex(PRODUCER_PESTS_RELATED_COL))
                        disease_related = cursor.getString(cursor.getColumnIndex(PRODUCER_DISEASE_RELATED_COL))
                        quality_related = cursor.getString(cursor.getColumnIndex(PRODUCER_QUALITY_RELATED_COL))
                        market_related = cursor.getString(cursor.getColumnIndex(PRODUCER_MARKET_RELATED_COL))
                        food_loss_related = cursor.getString(cursor.getColumnIndex(PRODUCER_FOOD_LOSS_RELATED_COL))
                        finance_related = cursor.getString(cursor.getColumnIndex(PRODUCER_FINANCE_RELATED_COL))
                        weather_related = cursor.getString(cursor.getColumnIndex(PRODUCER_WEATHER_RELATED_COL))
                        dairy_cattle = cursor.getString(cursor.getColumnIndex(PRODUCER_DAIRY_CATTLE_COL))
                        beef_cattle = cursor.getString(cursor.getColumnIndex(PRODUCER_BEEF_CATTLE_COL))
                        sheep = cursor.getString(cursor.getColumnIndex(PRODUCER_SHEEP_COL))
                        poultry = cursor.getString(cursor.getColumnIndex(PRODUCER_POULTRY_COL))
                        pigs = cursor.getString(cursor.getColumnIndex(PRODUCER_PIGS_COL))
                        rabbits = cursor.getString(cursor.getColumnIndex(PRODUCER_RABBITS_COL))
                        beehives = cursor.getString(cursor.getColumnIndex(PRODUCER_BEEHIVES_COL))
                        donkeys = cursor.getString(cursor.getColumnIndex(PRODUCER_DONKEYS_COL))
                        goats = cursor.getString(cursor.getColumnIndex(PRODUCER_GOATS_COL))
                        camels = cursor.getString(cursor.getColumnIndex(PRODUCER_CAMELS_COL))
                        aquaculture = cursor.getString(cursor.getColumnIndex(PRODUCER_AQUACULTURE_COL))
                        housing_type = cursor.getString(cursor.getColumnIndex(PRODUCER_HOUSING_TYPE_COL))
                        housing_floor = cursor.getString(cursor.getColumnIndex(PRODUCER_HOUSING_FLOOR_COL))
                        housing_roof = cursor.getString(cursor.getColumnIndex(PRODUCER_HOUSING_ROOF_COL))
                        lighting_fuel = cursor.getString(cursor.getColumnIndex(PRODUCER_LIGHTING_FUEL_COL))
                        cooking_fuel = cursor.getString(cursor.getColumnIndex(PRODUCER_COOKING_FUEL_COL))
                        water_filter = cursor.getString(cursor.getColumnIndex(PRODUCER_WATER_FILTER_COL))
                        water_tank_greater_than_5000lts = cursor.getString(cursor.getColumnIndex(PRODUCER_WATER_TANK_GREATER_THAN_5000LTS_COL))
                        hand_washing_facilities = cursor.getString(cursor.getColumnIndex(PRODUCER_HAND_WASHING_FACILITIES_COL))
                        ppes = cursor.getString(cursor.getColumnIndex(PRODUCER_PPES_COL))
                        water_well_or_weir = cursor.getString(cursor.getColumnIndex(PRODUCER_WATER_WELL_OR_WEIR_COL))
                        irrigation_pump = cursor.getString(cursor.getColumnIndex(PRODUCER_IRRIGATION_PUMP_COL))
                        harvesting_equipment = cursor.getString(cursor.getColumnIndex(PRODUCER_HARVESTING_EQUIPMENT_COL))
                        transportation_type = cursor.getString(cursor.getColumnIndex(PRODUCER_TRANSPORTATION_TYPE_COL))
                        toilet_floor = cursor.getString(cursor.getColumnIndex(PRODUCER_TOILET_FLOOR_COL))
                        user_approved = cursor.getInt(cursor.getColumnIndex(PRODUCER_USER_APPROVED_COL)) == 1
                        ta = cursor.getString(cursor.getColumnIndex(PRODUCER_TA_COL))
                        user_id = cursor.getString(cursor.getColumnIndex(PRODUCER_USER_ID_COL))
                    }

                    // Get associated commercial and domestic produces
                    val commercialProduces: List<CommercialProduce> = getCommercialProducesForProducer(producer.id)
                    val domesticProduces: List<DomesticProduce> = getDomesticProducesForProducer(producer.id)
                    producer.commercialProduces = commercialProduces
                    producer.domesticProduces = domesticProduces
    ...
    @SuppressLint("Range")
    fun getCommercialProducesForProducer(producerId: Int): List<CommercialProduce> {
        val commercialProduceList: MutableList<CommercialProduce> = ArrayList()
        val db = this.readableDatabase
        val columns = arrayOf(

            COMMERCIAL_PRODUCE_ID_COL,
            COMMERCIAL_PRODUCE_PRODUCT_COL,
            COMMERCIAL_PRODUCE_PRODUCT_CATEGORY_COL,
            COMMERCIAL_PRODUCE_ACERAGE_COL,
            COMMERCIAL_PRODUCER_BIODATA_ID_FK_COL
        )
        val selection = "$COMMERCIAL_PRODUCER_BIODATA_ID_FK_COL=?"
        val selectionArgs = arrayOf(producerId.toString())

        val cursor = db.query(COMMERCIAL_PRODUCE_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
        if (cursor != null && cursor.moveToFirst()) {
            do {
                val commercialProduce = CommercialProduce().apply {
                    id = cursor.getInt(cursor.getColumnIndex(COMMERCIAL_PRODUCE_ID_COL))
                    product = cursor.getString(cursor.getColumnIndex(COMMERCIAL_PRODUCE_PRODUCT_COL))
                    product_category = cursor.getString(cursor.getColumnIndex(COMMERCIAL_PRODUCE_PRODUCT_CATEGORY_COL))
                    acerage = cursor.getString(cursor.getColumnIndex(COMMERCIAL_PRODUCE_ACERAGE_COL))
                }

                // Add commercial produce to list
                commercialProduceList.add(commercialProduce)
            } while (cursor.moveToNext())

            cursor.close()
        }

        return commercialProduceList
    }

    @SuppressLint("Range")
    fun getDomesticProducesForProducer(producerId: Int): List<DomesticProduce> {
        val domesticProduceList: MutableList<DomesticProduce> = ArrayList()
        val db = this.readableDatabase
        val columns = arrayOf(
            DOMESTIC_PRODUCE_ID_COL,
            DOMESTIC_PRODUCE_PRODUCT_COL,
            DOMESTIC_PRODUCE_PRODUCT_CATEGORY_COL,
            DOMESTIC_PRODUCE_ACERAGE_COL,
            DOMESTIC_PRODUCER_BIODATA_ID_FK_COL
        )
        val selection = "$DOMESTIC_PRODUCER_BIODATA_ID_FK_COL=?"
        val selectionArgs = arrayOf(producerId.toString())

        val cursor = db.query(DOMESTIC_PRODUCE_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
        if (cursor != null && cursor.moveToFirst()) {
            do {
                val domesticProduce = DomesticProduce().apply {
                    id = cursor.getInt(cursor.getColumnIndex(DOMESTIC_PRODUCE_ID_COL))
                    product = cursor.getString(cursor.getColumnIndex(COMMERCIAL_PRODUCE_PRODUCT_COL))
                    product_category = cursor.getString(cursor.getColumnIndex(COMMERCIAL_PRODUCE_PRODUCT_CATEGORY_COL))
                    acerage = cursor.getString(cursor.getColumnIndex(COMMERCIAL_PRODUCE_ACERAGE_COL))
                }

                // Add domestic produce to list
                domesticProduceList.add(domesticProduce)
            } while (cursor.moveToNext())

            cursor.close()
        }

        return domesticProduceList
    }

    // Offline cig producer
    @get:SuppressLint("Range")
    val offlineCIGProducerBiodata: List<ProducerBiodata>
        get() {
            val producerList: MutableList<ProducerBiodata> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                CIG_PRODUCER_BIODATA_ID_COL,
                CIG_PRODUCER_OTHER_NAME_COL,
                CIG_PRODUCER_LAST_NAME_COL,
                CIG_PRODUCER_FARMER_CODE_COL,
                CIG_PRODUCER_ID_NUMBER_COL,
                CIG_PRODUCER_GENDER_COL,
                CIG_PRODUCER_DATE_OF_BIRTH_COL,
                CIG_PRODUCER_EMAIL_COL,
                CIG_PRODUCER_PHONE_NUMBER_COL,
                CIG_PRODUCER_HUB_COL,
                CIG_PRODUCER_BUYING_CENTER_COL,
                CIG_PRODUCER_EDUCATION_LEVEL_COL,
                CIG_PRODUCER_COUNTY_COL,
                CIG_PRODUCER_SUB_COUNTY_COL,
                CIG_PRODUCER_WARD_COL,
                CIG_PRODUCER_VILLAGE_COL,
                CIG_PRODUCER_PRIMARY_PRODUCER_COL,
                CIG_PRODUCER_TOTAL_LAND_SIZE_COL,
                CIG_PRODUCER_CULTIVATE_LAND_SIZE_COL,
                CIG_PRODUCER_HOMESTEAD_SIZE_COL,
                CIG_PRODUCER_UNCULTIVATED_LAND_SIZE_COL,
                CIG_PRODUCER_FARM_ACCESSIBILITY_COL,
                CIG_PRODUCER_NUMBER_OF_FAMILY_WORKERS_COL,
                CIG_PRODUCER_NUMBER_OF_HIRED_WORKERS_COL,
                CIG_PRODUCER_ACCESS_TO_IRRIGATION_COL,
                CIG_PRODUCER_CROP_LIST_COL,
                CIG_PRODUCER_FARMER_INTEREST_IN_EXTENSION_COL,
                CIG_PRODUCER_KNOWLEDGE_RELATED_COL,
                CIG_PRODUCER_SOIL_RELATED_COL,
                CIG_PRODUCER_COMPOST_RELATED_COL,
                CIG_PRODUCER_NUTRITION_RELATED_COL,
                CIG_PRODUCER_PESTS_RELATED_COL,
                CIG_PRODUCER_DISEASE_RELATED_COL,
                CIG_PRODUCER_QUALITY_RELATED_COL,
                CIG_PRODUCER_MARKET_RELATED_COL,
                CIG_PRODUCER_FOOD_LOSS_RELATED_COL,
                CIG_PRODUCER_FINANCE_RELATED_COL,
                CIG_PRODUCER_WEATHER_RELATED_COL,
                CIG_PRODUCER_DAIRY_CATTLE_COL,
                CIG_PRODUCER_BEEF_CATTLE_COL,
                CIG_PRODUCER_SHEEP_COL,
                CIG_PRODUCER_POULTRY_COL,
                CIG_PRODUCER_PIGS_COL,
                CIG_PRODUCER_RABBITS_COL,
                CIG_PRODUCER_BEEHIVES_COL,
                CIG_PRODUCER_DONKEYS_COL,
                CIG_PRODUCER_GOATS_COL,
                CIG_PRODUCER_CAMELS_COL,
                CIG_PRODUCER_AQUACULTURE_COL,
                CIG_PRODUCER_HOUSING_TYPE_COL,
                CIG_PRODUCER_HOUSING_FLOOR_COL,
                CIG_PRODUCER_HOUSING_ROOF_COL,
                CIG_PRODUCER_LIGHTING_FUEL_COL,
                CIG_PRODUCER_COOKING_FUEL_COL,
                CIG_PRODUCER_WATER_FILTER_COL,
                CIG_PRODUCER_WATER_TANK_GREATER_THAN_5000LTS_COL,
                CIG_PRODUCER_HAND_WASHING_FACILITIES_COL,
                CIG_PRODUCER_PPES_COL,
                CIG_PRODUCER_WATER_WELL_OR_WEIR_COL,
                CIG_PRODUCER_IRRIGATION_PUMP_COL,
                CIG_PRODUCER_HARVESTING_EQUIPMENT_COL,
                CIG_PRODUCER_TRANSPORTATION_TYPE_COL,
                CIG_PRODUCER_TOILET_FLOOR_COL,
                CIG_PRODUCER_USER_APPROVED_COL,
                CIG_PRODUCER_TA_COL,
                CIG_PRODUCER_USER_ID_COL,
                IS_OFFLINE_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1")

            val cursor = db.query(CIG_PRODUCER_BIODATA_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val producer = ProducerBiodata().apply {
                        val primaryProducerString = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_PRIMARY_PRODUCER_COL))

                        id = cursor.getInt(cursor.getColumnIndex(CIG_PRODUCER_BIODATA_ID_COL))
                        other_name = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_OTHER_NAME_COL))
                        last_name = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_LAST_NAME_COL))
                        farmer_code = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_FARMER_CODE_COL))
                        id_number = cursor.getString(cursor.getColumnIndex(PRODUCER_ID_NUMBER_COL))
                        gender = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_GENDER_COL))
                        date_of_birth = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_DATE_OF_BIRTH_COL))
                        email = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_EMAIL_COL))
                        phone_number = cursor.getString(cursor.getColumnIndex(PRODUCER_PHONE_NUMBER_COL))
                        hub = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_HUB_COL))
                        buying_center = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_BUYING_CENTER_COL))
                        education_level = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_EDUCATION_LEVEL_COL))
                        county = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_COUNTY_COL))
                        sub_county = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_SUB_COUNTY_COL))
                        ward = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_WARD_COL))
                        village = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_VILLAGE_COL))
                        ...
                        total_land_size = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_TOTAL_LAND_SIZE_COL))
                        cultivate_land_size = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_CULTIVATE_LAND_SIZE_COL))
                        homestead_size = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_HOMESTEAD_SIZE_COL))
                        uncultivated_land_size = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_UNCULTIVATED_LAND_SIZE_COL))
                        farm_accessibility = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_FARM_ACCESSIBILITY_COL))
                        number_of_family_workers = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_NUMBER_OF_FAMILY_WORKERS_COL))
                        number_of_hired_workers = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_NUMBER_OF_HIRED_WORKERS_COL))
                        access_to_irrigation = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_ACCESS_TO_IRRIGATION_COL))
                        crop_list = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_CROP_LIST_COL))
                        farmer_interest_in_extension = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_FARMER_INTEREST_IN_EXTENSION_COL))
                        knowledge_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_KNOWLEDGE_RELATED_COL))
                        soil_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_SOIL_RELATED_COL))
                        compost_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_COMPOST_RELATED_COL))
                        nutrition_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_NUTRITION_RELATED_COL))
                        pests_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_PESTS_RELATED_COL))
                        disease_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_DISEASE_RELATED_COL))
                        quality_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_QUALITY_RELATED_COL))
                        market_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_MARKET_RELATED_COL))
                        food_loss_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_FOOD_LOSS_RELATED_COL))
                        finance_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_FINANCE_RELATED_COL))
                        weather_related = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_WEATHER_RELATED_COL))
                        dairy_cattle = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_DAIRY_CATTLE_COL))
                        beef_cattle = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_BEEF_CATTLE_COL))
                        sheep = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_SHEEP_COL))
                        poultry = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_POULTRY_COL))
                        pigs = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_PIGS_COL))
                        rabbits = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_RABBITS_COL))
                        beehives = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_BEEHIVES_COL))
                        donkeys = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_DONKEYS_COL))
                        goats = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_GOATS_COL))
                        camels = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_CAMELS_COL))
                        aquaculture = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_AQUACULTURE_COL))
                        housing_type = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_HOUSING_TYPE_COL))
                        housing_floor = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_HOUSING_FLOOR_COL))
                        housing_roof = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_HOUSING_ROOF_COL))
                        lighting_fuel = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_LIGHTING_FUEL_COL))
                        cooking_fuel = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_COOKING_FUEL_COL))
                        water_filter = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_WATER_FILTER_COL))
                        water_tank_greater_than_5000lts = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_WATER_TANK_GREATER_THAN_5000LTS_COL))
                        hand_washing_facilities = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_HAND_WASHING_FACILITIES_COL))
                        ppes = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_PPES_COL))
                        water_well_or_weir = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_WATER_WELL_OR_WEIR_COL))
                        irrigation_pump = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_IRRIGATION_PUMP_COL))
                        harvesting_equipment = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_HARVESTING_EQUIPMENT_COL))
                        transportation_type = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_TRANSPORTATION_TYPE_COL))
                        toilet_floor = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_TOILET_FLOOR_COL))
                        user_approved = cursor.getInt(cursor.getColumnIndex(CIG_PRODUCER_USER_APPROVED_COL)) == 1
                        ta = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_TA_COL))
                        user_id = cursor.getString(cursor.getColumnIndex(CIG_PRODUCER_USER_ID_COL))
                    }

                    // Get associated commercial and domestic produces
                    val commercialProduces: List<CommercialProduce> = getCIGCommercialProducesForProducer(producer.id)
                    val domesticProduces: List<DomesticProduce> = getCIGDomesticProducesForProducer(producer.id)
                    producer.commercialProduces = commercialProduces
                    producer.domesticProduces = domesticProduces
    ...
    @SuppressLint("Range")
    fun getCIGCommercialProducesForProducer(producerId: Int): List<CommercialProduce> {
        val commercialProduceList: MutableList<CommercialProduce> = ArrayList()
        val db = this.readableDatabase
        val columns = arrayOf(
            ...
            COMMERCIAL_PRODUCE_PRODUCT_CATEGORY_COL,
            COMMERCIAL_PRODUCE_ACERAGE_COL,
            COMMERCIAL_CIG_PRODUCER_BIODATA_ID_FK_COL
        )
        val selection = "$COMMERCIAL_CIG_PRODUCER_BIODATA_ID_FK_COL=?"
        val selectionArgs = arrayOf(producerId.toString())
    ...
    @SuppressLint("Range")
    fun getCIGDomesticProducesForProducer(producerId: Int): List<DomesticProduce> {
        val domesticProduceList: MutableList<DomesticProduce> = ArrayList()
        val db = this.readableDatabase
        val columns = arrayOf(
            DOMESTIC_PRODUCE_ID_COL,
            ...
            DOMESTIC_PRODUCE_PRODUCT_CATEGORY_COL,
            DOMESTIC_PRODUCE_ACERAGE_COL,
            DOMESTIC_CIG_PRODUCER_BIODATA_ID_FK_COL
        )
        val selection = "$DOMESTIC_CIG_PRODUCER_BIODATA_ID_FK_COL=?"
        val selectionArgs = arrayOf(producerId.toString())
...
//
    ...
    @get:SuppressLint("Range")
    val offlineSeasonPlanning: List<SeasonPlanning>
        get() {
            val seasonPlanningList = mutableListOf<SeasonPlanning>()
            val db = this.readableDatabase
            val columns = arrayOf(
                SEASON_PLANNING_ID_COL,
                SEASON_PLANNING_PRODUCER_COL,
                SEASON_PLANNING_FIELD_COL,
                SEASON_PLANNING_PLANNED_DATE_OF_PLANTING_COL,
                SEASON_PLANNING_WEEK_NUMBER_COL,
                SEASON_PLANNING_NURSERY_COL,
                SEASON_PLANNING_GAPPING_COL,
                SEASON_PLANNING_SOIL_ANALYSIS_COL,
                SEASON_PLANNING_LIMING_COL,
                SEASON_PLANNING_TRANSPLANTING_COL,
                SEASON_PLANNING_WEEDING_COL,
                SEASON_PLANNING_PRUNNING_THINNING_DESUCKERING_COL,
                SEASON_PLANNING_MULCHING_COL,
                SEASON_PLANNING_HARVESTING_COL,
                SEASON_PLANNING_USER_ID_FK_COL,
                IS_OFFLINE_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1")
            ...
            val cursor = db.query(SEASON_PLANNING_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val seasonPlanning = SeasonPlanning().apply {
                        id = cursor.getInt(cursor.getColumnIndex(SEASON_PLANNING_ID_COL))
                        producer = cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_PRODUCER_COL))

                        // Log column index and value for debugging
                        fieldName = cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_FIELD_COL))

                        planned_date_of_planting = cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_PLANNED_DATE_OF_PLANTING_COL))
                        week_number = cursor.getInt(cursor.getColumnIndex(SEASON_PLANNING_WEEK_NUMBER_COL))

                        // Handle JSON fields
                        nursery = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_NURSERY_COL)))
                        gapping = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_GAPPING_COL)))
                        soil_analysis = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_SOIL_ANALYSIS_COL)))
                        liming = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_LIMING_COL)))
                        transplanting = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_TRANSPLANTING_COL)))
                        weeding = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_WEEDING_COL)))
                        prunning_thinning_desuckering = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_PRUNNING_THINNING_DESUCKERING_COL)))
                        mulching = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_MULCHING_COL)))
                        harvesting = parseJsonObject(cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_HARVESTING_COL)))
                        user_id = cursor.getString(cursor.getColumnIndex(SEASON_PLANNING_USER_ID_FK_COL))
                    }

                    // Get associated data
                    val marketProduces: List<MarketProduce> = getMarketProducesForSeasonPlanning(seasonPlanning.id)
                    val planNutritions: List<PlanNutrition> = getPlanNutritionsForSeasonPlanning(seasonPlanning.id)
                    val scoutingStations: List<ScoutingStation> = getScoutingStationsForSeasonPlanning(seasonPlanning.id)
                    val preventativePests: List<PreventativePest> = getPreventativePestsForSeasonPlanning(seasonPlanning.id)
                    val preventativeDiseases: List<PreventativeDisease> = getPreventativeDiseasesForSeasonPlanning(seasonPlanning.id)
                    val planIrrigations: List<PlanIrrigation> = getPlanIrrigationForSeasonPlanning(seasonPlanning.id)

                    // Set associated data to season planning
                    seasonPlanning.marketProduces = marketProduces
                    seasonPlanning.plan_nutritions = planNutritions
                    seasonPlanning.scouting_stations = scoutingStations
                    seasonPlanning.preventative_diseases = preventativeDiseases
                    seasonPlanning.preventative_pests = preventativePests
                    seasonPlanning.plan_irrigations = planIrrigations

                    // Add season planning to list
                    seasonPlanningList.add(seasonPlanning)
                } while (cursor.moveToNext())

                cursor.close()
            }
            return seasonPlanningList
        }
    ...
    fun deleteSeasonPlanning(seasonPlanningId: Int) {
        val db = this.writableDatabase
        val whereClause = "$SEASON_PLANNING_ID_COL=?"
        val whereArgs = arrayOf(seasonPlanningId.toString())

        val rowsDeleted = db.delete(SEASON_PLANNING_TABLE_NAME, whereClause, whereArgs)
        if (rowsDeleted > 0) {
            Log.d("PostSeasonPlanningData", "Successfully deleted season planning with ID: $seasonPlanningId")
        } else {
            Log.e("PostSeasonPlanningData", "Failed to delete season planning with ID: $seasonPlanningId")
        }
    }

    // Offline extension services
    @get:SuppressLint("Range")
    val offlineExtensionServices: List<ExtensionService>
        get() {
            val extensionServiceList = mutableListOf<ExtensionService>()
            val db = this.readableDatabase
            val columns = arrayOf(
                EXTENSION_SERVICE_ID_COL,
                EXTENSION_SERVICE_PRODUCER_COL,
                EXTENSION_SERVICE_FIELD_COL,
                EXTENSION_SERVICE_PLANNED_DATE_OF_PLANTING_COL,
                EXTENSION_SERVICE_WEEK_NUMBER_COL,
                EXTENSION_SERVICE_NURSERY_COL,
                EXTENSION_SERVICE_GAPPING_COL,
                EXTENSION_SERVICE_SOIL_ANALYSIS_COL,
                EXTENSION_SERVICE_LIMING_COL,
                EXTENSION_SERVICE_TRANSPLANTING_COL,
                EXTENSION_SERVICE_WEEDING_COL,
                EXTENSION_SERVICE_PRUNNING_THINNING_DESUCKERING_COL,
                EXTENSION_SERVICE_MULCHING_COL,
                EXTENSION_SERVICE_HARVESTING_COL,
                EXTENSION_SERVICE_USER_ID_FK_COL,
                IS_OFFLINE_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1")

            val cursor = db.query(EXTENSION_SERVICE_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val extensionService = ExtensionService(
                        id = cursor.getInt(cursor.getColumnIndex(EXTENSION_SERVICE_ID_COL)),
                        producer = cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_PRODUCER_COL)),
                        fieldName = cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_FIELD_COL)),
                        planned_date_of_planting = cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_PLANNED_DATE_OF_PLANTING_COL)),
                        week_number = cursor.getInt(cursor.getColumnIndex(EXTENSION_SERVICE_WEEK_NUMBER_COL)),
                        nursery = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_NURSERY_COL))),
                        gapping = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_GAPPING_COL))),
                        soil_analysis = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_SOIL_ANALYSIS_COL))),
                        liming = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_LIMING_COL))),
                        transplanting = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_TRANSPLANTING_COL))),
                        weeding = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_WEEDING_COL))),
                        prunning_thinning_desuckering = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_PRUNNING_THINNING_DESUCKERING_COL))),
                        mulching = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_MULCHING_COL))),
                        harvesting = parseJsonObject(cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_HARVESTING_COL))),
                        user_id = cursor.getString(cursor.getColumnIndex(EXTENSION_SERVICE_USER_ID_FK_COL))
                    )

                    // Fetch and set associated data
                    extensionService.ext_scouting_stations = getExtScoutingStationsForExtensionService(extensionService.id)
                    extensionService.pesticides_used = getPesticidesUsedForExtensionService(extensionService.id)
                    extensionService.fertlizers_used = getFertilizersUsedForExtensionService(extensionService.id)
                    extensionService.forecast_yields = getForecastYieldsForExtensionService(extensionService.id)
                    extensionService.marketProduces = getMarketProducesForExtensionService(extensionService.id)

                    extensionServiceList.add(extensionService)
                } while (cursor.moveToNext())

                cursor.close()
            }
            return extensionServiceList
        }

    // Helper function to safely query the database and map results
    private fun <T> queryDatabase(
        tableName: String,
        columnName: String,
        extensionServiceId: Int,
        mapper: (Cursor) -> T
    ): List<T> {
        val results = mutableListOf<T>()
        val db = this.readableDatabase
        val query = "SELECT * FROM $tableName WHERE $columnName = ?"
        db.rawQuery(query, arrayOf(extensionServiceId.toString())).use { cursor ->
            while (cursor.moveToNext()) {
    ...
    fun deleteExtensionService(extensionServiceId: Int) {
        val db = this.writableDatabase
        val whereClause = "$EXTENSION_SERVICE_ID_COL=?"
        val whereArgs = arrayOf(extensionServiceId.toString())

        val rowsDeleted = db.delete(EXTENSION_SERVICE_TABLE_NAME, whereClause, whereArgs)
        if (rowsDeleted > 0) {
            Log.d("PostExtensionServiceData", "Successfully deleted extension service with ID: $extensionServiceId")
        } else {
            Log.e("PostExtensionServiceData", "Failed to delete extension service with ID: $extensionServiceId")
        }
    }
    ...
    @get:SuppressLint("Range")
    val offlineQuarantines: List<Quarantine>
        get() {
            val quarantineList: MutableList<Quarantine> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                QUARANTINE_ID_COL,
                QUARANTINE_ACTION_COL,
                QUARANTINE_APPROVED_BY_COL,
                QUARANTINE_NEW_WEIGHT_IN_COL,
                QUARANTINE_NEW_WEIGHT_OUT_COL,
                QUARANTINE_BUYING_FARMER_ID_FK_COL,
                QUARANTINE_BUYING_CUSTOMER_ID_FK_COL,
                IS_OFFLINE_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1")

            val cursor = db.query(QUARANTINE_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val quarantine = Quarantine().apply {
                        id = cursor.getInt(cursor.getColumnIndex(QUARANTINE_ID_COL))
                        action = cursor.getString(cursor.getColumnIndex(QUARANTINE_ACTION_COL))
                        quarantine_approved_by = cursor.getString(cursor.getColumnIndex(QUARANTINE_APPROVED_BY_COL))
                        new_weight_in_after_sorting_or_regrading = cursor.getString(cursor.getColumnIndex(QUARANTINE_NEW_WEIGHT_IN_COL))
                        new_weight_out_after_sorting_or_regrading = cursor.getString(cursor.getColumnIndex(QUARANTINE_NEW_WEIGHT_OUT_COL))
                        buying_farmer_id = cursor.getInt(cursor.getColumnIndex(QUARANTINE_BUYING_FARMER_ID_FK_COL))
                        buying_customer_id = cursor.getInt(cursor.getColumnIndex(QUARANTINE_BUYING_CUSTOMER_ID_FK_COL))
                    }

                    // Add quarantine to list
    ...
    @get:SuppressLint("Range")
    val offlinePaymentFarmers: List<PaymentFarmer>
        get() {
            val paymentFarmerList: MutableList<PaymentFarmer> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                PAYMENT_FARMER_ID_COL,
                PAYMENT_FARMER_BUYING_CENTER_COL,
                PAYMENT_FARMER_CIG_COL,
                PAYMENT_FARMER_PRODUCER_COL,
                PAYMENT_FARMER_GRN_COL,
                PAYMENT_FARMER_NET_BALANCE_COL,
                PAYMENT_FARMER_PAYMENT_TYPE_COL,
                PAYMENT_FARMER_OUTSTANDING_LOAN_AMOUNT_COL,
                PAYMENT_FARMER_PAYMENT_DUE_COL,
                PAYMENT_FARMER_SET_LOAN_DEDUCTION_COL,
                PAYMENT_FARMER_NET_BALANCE_BEFORE_COL,
                PAYMENT_FARMER_NET_BALANCE_AFTER_LOAN_DEDUCTION_COL,
                PAYMENT_FARMER_COMMENT_COL,
                PAYMENT_FARMER_USER_ID_FK_COL,
                IS_OFFLINE_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1")

            val cursor = db.query(PAYMENT_FARMER_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val paymentFarmer = PaymentFarmer().apply {
                        id = cursor.getInt(cursor.getColumnIndex(PAYMENT_FARMER_ID_COL))
                        buying_center = cursor.getString(cursor.getColumnIndex(PAYMENT_FARMER_BUYING_CENTER_COL))
                        cig = cursor.getString(cursor.getColumnIndex(PAYMENT_FARMER_CIG_COL))
                        producer = cursor.getString(cursor.getColumnIndex(PAYMENT_FARMER_PRODUCER_COL))
                        grn = cursor.getString(cursor.getColumnIndex(PAYMENT_FARMER_GRN_COL))
                        net_balance = cursor.getDouble(cursor.getColumnIndex(PAYMENT_FARMER_NET_BALANCE_COL))
                        payment_type = cursor.getString(cursor.getColumnIndex(PAYMENT_FARMER_PAYMENT_TYPE_COL))
                        outstanding_loan_amount = cursor.getDouble(cursor.getColumnIndex(PAYMENT_FARMER_OUTSTANDING_LOAN_AMOUNT_COL))
                        payment_due = cursor.getDouble(cursor.getColumnIndex(PAYMENT_FARMER_PAYMENT_DUE_COL))
                        set_loan_deduction = cursor.getDouble(cursor.getColumnIndex(PAYMENT_FARMER_SET_LOAN_DEDUCTION_COL))
                        net_balance_before = cursor.getDouble(cursor.getColumnIndex(PAYMENT_FARMER_NET_BALANCE_BEFORE_COL))
                        net_balance_after_loan_deduction = cursor.getDouble(cursor.getColumnIndex(PAYMENT_FARMER_NET_BALANCE_AFTER_LOAN_DEDUCTION_COL))
                        comment = cursor.getString(cursor.getColumnIndex(PAYMENT_FARMER_COMMENT_COL))
                        user_id = cursor.getString(cursor.getColumnIndex(PAYMENT_FARMER_USER_ID_FK_COL))
                    }

                    // Add paymentFarmer to list
                    paymentFarmerList.add(paymentFarmer)
                } while (cursor.moveToNext())

                cursor.close()
            }

            return paymentFarmerList
        }

    @get:SuppressLint("Range")
    val offlinePaymentCustomers: List<PaymentCustomer>
        get() {
            val paymentCustomerList: MutableList<PaymentCustomer> = ArrayList()
            val db = this.readableDatabase
            val columns = arrayOf(
                PAYMENT_CUSTOMER_ID_COL,
                PAYMENT_CUSTOMER_VILLAGE_OR_ESTATE_COL,
                PAYMENT_CUSTOMER_CUSTOMER_COL,
                PAYMENT_CUSTOMER_GRN_COL,
                PAYMENT_CUSTOMER_AMOUNT_COL,
                PAYMENT_CUSTOMER_NET_BALANCE_COL,
                PAYMENT_CUSTOMER_PAYMENT_TYPE_COL,
                PAYMENT_CUSTOMER_ENTER_AMOUNT_COL,
                PAYMENT_CUSTOMER_NET_BALANCE_BEFORE_COL,
                PAYMENT_CUSTOMER_NET_BALANCE_AFTER_COL,
                PAYMENT_CUSTOMER_COMMENT_COL,
                IS_OFFLINE_COL,
                PAYMENT_CUSTOMER_USER_ID_FK_COL
            )
            val selection = "$IS_OFFLINE_COL=?"
            val selectionArgs = arrayOf("1")

            val cursor = db.query(PAYMENT_CUSTOMER_TABLE_NAME, columns, selection, selectionArgs, null, null, null)
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    val paymentCustomer = PaymentCustomer().apply {
                        id = cursor.getInt(cursor.getColumnIndex(PAYMENT_CUSTOMER_ID_COL))
                        village_or_estate = cursor.getString(cursor.getColumnIndex(PAYMENT_CUSTOMER_VILLAGE_OR_ESTATE_COL))
                        customer = cursor.getString(cursor.getColumnIndex(PAYMENT_CUSTOMER_CUSTOMER_COL))
                        grn = cursor.getString(cursor.getColumnIndex(PAYMENT_CUSTOMER_GRN_COL))
                        amount = cursor.getDouble(cursor.getColumnIndex(PAYMENT_CUSTOMER_AMOUNT_COL))
                        net_balance = cursor.getDouble(cursor.getColumnIndex(PAYMENT_CUSTOMER_NET_BALANCE_COL))
                        payment_type = cursor.getString(cursor.getColumnIndex(PAYMENT_CUSTOMER_PAYMENT_TYPE_COL))
                        amount = cursor.getDouble(cursor.getColumnIndex(PAYMENT_CUSTOMER_ENTER_AMOUNT_COL))
                        net_balance_before = cursor.getDouble(cursor.getColumnIndex(PAYMENT_CUSTOMER_NET_BALANCE_BEFORE_COL))
                        net_balance_after = cursor.getDouble(cursor.getColumnIndex(PAYMENT_CUSTOMER_NET_BALANCE_AFTER_COL))
                        comment = cursor.getString(cursor.getColumnIndex(PAYMENT_CUSTOMER_COMMENT_COL))
                        user_id = cursor.getString(cursor.getColumnIndex(PAYMENT_CUSTOMER_USER_ID_FK_COL))
                    }

                    // Add paymentCustomer to list
                    paymentCustomerList.add(paymentCustomer)
                } while (cursor.moveToNext())
    ...
    fun deletePlanJourney(id: Int) {
        val db = writableDatabase
        db.delete(PLAN_JOURNEY_TABLE_NAME, "$PLAN_JOURNEY_ID_COL = ?", arrayOf(id.toString()))
    }
