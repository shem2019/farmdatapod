...
class CropProtectionRepository(private val context: Context) : SyncableRepository {
    private val TAG = "CropProtectionRepo"
        ...
        try {
            ...
            if (isOnline) {
                try {
                    ...
                    context?.let { ctx ->
                        RestClient.getApiService(ctx).planCropProtection(protectionModel)
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            return@withContext suspendCancellableCoroutine { continuation ->
                context?.let { ctx ->
                    RestClient.getApiService(ctx).getCropProtection()
