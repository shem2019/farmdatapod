...
class CropManagementRepository(private val context: Context) : SyncableRepository {
    private val TAG = "CropManagementRepository"
    ...
    suspend fun saveCropManagement(
        ...
        try {
            ...
            // Check if activity already exists based on relevant fields
            ...
            if (isOnline) {
                try {
                    ...
                    context?.let { ctx ->
                        RestClient.getApiService(ctx).planCropManagementActivities(cropManagement)
                            .enqueue(object : Callback<CropManagementModel> {
                                    ...
                                    if (response.isSuccessful) {
                                        response.body()?.let { serverResponse ->
                                            CoroutineScope(Dispatchers.IO).launch {
                                                ...
                                                Log.d(TAG, "Crop management synced with server ID: ${serverResponse.id}")
                                            }
                                        }
                                    } else {
                                ...
                                override fun onFailure(call: Call<CropManagementModel>, t: Throwable) {
        ...
        try {
            ...
            unsyncedActivities.forEach { activityWithRelations ->
                try {
                    ...
                    context?.let { ctx ->
                        RestClient.getApiService(ctx).planCropManagementActivities(cropManagementModel)
                            .enqueue(object : Callback<CropManagementModel> {
                                override fun onResponse(
                                    call: Call<CropManagementModel>,
                                    response: Response<CropManagementModel>
                                        ...
                                        response.body()?.let { serverResponse ->
                                            CoroutineScope(Dispatchers.IO).launch {
                                                cropManagementDao.markAsSynced(
                                                    ...
                                                    serverResponse.id?.toString(), // Convert to String if it's not already
                                                    System.currentTimeMillis()
                                                )
                                                successCount++
                                                Log.d(TAG, "Successfully synced activity ID: ${activityWithRelations.cropManagement.id}")
                                            }
                                        }
                                    } else {
                                        failureCount++
                                        Log.e(TAG, "Failed to sync activity ID: ${activityWithRelations.cropManagement.id}")
                                    }
                                }
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            ...
            return@withContext suspendCancellableCoroutine { continuation ->
                context?.let { ctx ->
                    RestClient.getApiService(ctx).getCropManagementActivities()
                        .enqueue(object : Callback<List<CropManagementModel>> {
                            override fun onResponse(
                                ...
                                if (response.isSuccessful) {
                                                ...
                                                activities.distinctBy { "${it.producer}:${it.season}:${it.field}:${it.date}" }
                                                    .forEach { serverActivity ->
                                                        try {
                                                            ...
                                                            Log.e(TAG, "Error processing activity", e)
                                                        }
                                                    }
                                            }
                                    ...
                                    when (response.code()) {
                                        401, 403 -> continuation.resume(Result.failure(SecurityException("Authentication error: ${response.code()}")))
                                        else -> continuation.resume(Result.failure(RuntimeException("Unexpected response: ${response.code()}")))
                                    }
                                }
                            }

                            override fun onFailure(call: Call<List<CropManagementModel>>, t: Throwable) {
    ...
    private fun shouldUpdate(existingActivity: CropManagementEntity, serverActivity: CropManagementModel): Boolean {
    ...
    private fun createUpdatedActivity(existingActivity: CropManagementEntity, serverActivity: CropManagementModel): CropManagementEntity {
    ...
    private fun convertToModel(entityWithRelations: CropManagementWithActivities): CropManagementModel {
        return CropManagementModel(
            ...
            gappingActivity = entityWithRelations.gappingActivities.firstOrNull()?.let { convertFromGappingEntity(it) }
                ?: createDefaultGappingActivity(),
            weedingActivity = entityWithRelations.weedingActivities.firstOrNull()?.let { convertFromWeedingEntity(it) }
                ?: createDefaultWeedingActivity(),
            pruningActivity = entityWithRelations.pruningActivities.firstOrNull()?.let { convertFromPruningEntity(it) }
                ?: createDefaultPruningActivity(),
            stakingActivity = entityWithRelations.stakingActivities.firstOrNull()?.let { convertFromStakingEntity(it) }
                ?: createDefaultStakingActivity(),
            thinningActivity = entityWithRelations.thinningActivities.firstOrNull()?.let { convertFromThinningEntity(it) }
                ?: createDefaultThinningActivity(),
            wateringActivity = entityWithRelations.wateringActivities.firstOrNull()?.let { convertFromWateringEntity(it) }
