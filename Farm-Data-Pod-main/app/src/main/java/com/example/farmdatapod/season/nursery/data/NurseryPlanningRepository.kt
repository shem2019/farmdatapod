...
class NurseryPlanningRepository(private val context: Context) : SyncableRepository {
    private val TAG = "NurseryPlanningRepository"
        ...
        try {
            ...
            if (isOnline) {
                try {
                    ...
                    context?.let { ctx ->
                        RestClient.getApiService(ctx).planNursery(planRequest)
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            ...
            return@withContext suspendCancellableCoroutine { continuation ->
                context?.let { ctx ->
                    RestClient.getApiService(ctx).getNurseryPlans()
