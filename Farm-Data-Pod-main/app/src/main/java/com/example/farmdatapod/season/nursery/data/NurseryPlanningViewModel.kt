...
class NurseryPlanningViewModel(application: Application) : AndroidViewModel(application) {
    private val repository = NurseryPlanningRepository(application)
    ...
    fun saveNurseryPlan(
        ...
        trayType: String,
        numberOfTrays: Int,
        seasonPlanningId: Long,  // Add this parameter
        comments: String?,
        ...
        viewModelScope.launch {
            try {
                val nurseryPlan = NurseryPlanEntity(
                    ...
                    comments = comments,
                    seasonPlanningId = seasonPlanningId,  // Use the passed ID
                    createdAt = Date(),
                    isUploaded = false
                )
