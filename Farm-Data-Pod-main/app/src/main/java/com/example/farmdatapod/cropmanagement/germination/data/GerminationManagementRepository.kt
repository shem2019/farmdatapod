package com.example.farmdatapod.cropmanagement.germination.data
...
class GerminationManagementRepository(private val context: Context) : SyncableRepository {
    private val TAG = "GerminationManagementRepository"
            ...
            try {
                ...
                if (isOnline) {
                    try {
                        ...
                        val response = apiService.postGerminationManagement(germinationRequest).execute()
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            ...
            val response = apiService.getGerminationManagement().execute()
