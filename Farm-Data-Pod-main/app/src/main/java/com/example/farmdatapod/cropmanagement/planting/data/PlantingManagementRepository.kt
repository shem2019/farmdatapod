package com.example.farmdatapod.cropmanagement.planting.data
...
import com.example.farmdatapod.season.planting.data.CompletePlantingPlan
...
class PlantingManagementRepository(private val context: Context) : SyncableRepository {
    private val TAG = "PlantingManagementRepo"
    ...
    suspend fun savePlanting(
        plan: PlanPlantingModel,
        isOnline: Boolean
    ): Result<CompletePlantingPlan> = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "Attempting to save planting for producer: ${plan.producer}, Online mode: $isOnline")
            ...
            if (isOnline) {
                try {
                    // Using the planting endpoint instead of plan-planting
                    val response = apiService.plantingManagement(planRequest).execute()
                    if (response.isSuccessful) {
                        plantingDao.markPlanAsSynced(planId)
                        Log.d(TAG, "Planting synced successfully with server")
                    } else {
            ...
            Log.e(TAG, "Error saving planting", e)
            Result.failure(e)
        }
    }
            ...
            unsyncedPlans.forEach { unsyncedPlan ->
                try {
                    ...
                    if (material != null && method != null) {
                        ...
                        // Using the planting endpoint instead of plan-planting
                        val response = apiService.plantingManagement(planPlantingModel).execute()
                        if (response.isSuccessful) {
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            // Using the planting endpoint instead of plan-planting
            val response = apiService.getPlantingManagement().execute()
