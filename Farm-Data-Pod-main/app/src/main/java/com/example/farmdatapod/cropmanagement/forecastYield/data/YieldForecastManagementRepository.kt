...
class YieldForecastManagementRepository(private val context: Context) : SyncableRepository {
    private val TAG = "YieldForecastManagementRepository"
    ...
    suspend fun saveYieldForecast(yieldForecast: YieldForecast, isOnline: Boolean): Result<YieldForecast> =
        withContext(Dispatchers.IO) {
            try {
                ...
                if (isOnline) {
                    try {
                        apiService.forecastYieldsManagement(createForecastRequest(yieldForecast))
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            val response = apiService.getForecastYieldsManagement().execute()
            when (response.code()) {
                200 -> {
    ...
    override suspend fun syncUnsynced(): Result<SyncStats> = withContext(Dispatchers.IO) {
        try {
            ...
            unsyncedForecasts.forEach { forecast ->
                try {
                    val response = apiService.forecastYieldsManagement(createForecastRequest(forecast)).execute()
                    if (response.isSuccessful) {
                        response.body()?.let { serverForecast ->
