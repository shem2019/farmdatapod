...
class BaitRepository(private val context: Context) : SyncableRepository {
    private val TAG = "BaitRepository"
            ...
            try {
                ...
                if (isOnline) {
                    try {
                        ...
                        val response = apiService.planScoutingStation(baitRequest).execute()
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            ...
            val response = apiService.getScoutingStations().execute()
