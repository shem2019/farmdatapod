package com.example.farmdatapod.cropmanagement.nutrition.data
...
import com.example.farmdatapod.season.nutrition.data.CropNutritionWithApplicants
...
class CropNutritionManagementRepository(private val context: Context) : SyncableRepository {
    private val TAG = "CropNutritionManagementRepository"
    ...
    suspend fun saveCropNutrition(
        ...
        try {
            Log.d(
                TAG,
                "Attempting to save crop nutrition: ${cropNutrition.product}, Online mode: $isOnline"
            )

            var savedNutrition = cropNutrition
            val localId =
                cropNutritionDao.insertCropNutritionWithApplicants(cropNutrition, applicants)
            ...
            if (isOnline) {
                try {
                    ...
                    context?.let { ctx ->
                        // Changed from planCropNutrition to CropNutritionManagement
                        RestClient.getApiService(ctx).CropNutritionManagement(nutritionModel)
                                    ...
                                    if (response.isSuccessful) {
                                        response.body()?.let { serverNutrition ->
                                            CoroutineScope(Dispatchers.IO).launch {
                                                ...
                                                Log.d(
                                                    TAG,
                                                    "Crop nutrition synced with server ID: ${serverNutrition.id}"
                                                )
                                            }
                                        }
                                    } else {
    ...
    override suspend fun syncUnsynced(): Result<SyncStats> = withContext(Dispatchers.IO) {
        try {
            ...
            unsyncedNutrition.forEach { nutrition ->
                try {
                    val applicants =
                        cropNutritionDao.getApplicantsByCropNutritionId(nutrition.id.toLong())
                                    ...
                                    if (response.isSuccessful) {
                                        response.body()?.let { serverNutrition ->
                                            CoroutineScope(Dispatchers.IO).launch {
                                                ...
                                                Log.d(
                                                    TAG,
                                                    "Successfully synced ${nutrition.product}"
                                                )
                                            }
                                        }
                                    } else {
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            return@withContext suspendCancellableCoroutine { continuation ->
                context?.let { ctx ->
                    // Changed from getCropNutrition to getCropNutritionManagement
                    RestClient.getApiService(ctx).getCropNutritionManagement()
                ...
                for (i in nameOfApplicants.name.indices) {
                    applicants.add(
                        ApplicantEntity(
                            ...
                            equipmentUsed = (nameOfApplicants.equipment_used.getOrNull(i)
                                ?: "").toString()
                        )
                    )
                }
