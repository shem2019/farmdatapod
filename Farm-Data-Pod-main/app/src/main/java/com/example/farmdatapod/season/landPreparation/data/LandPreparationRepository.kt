...
class LandPreparationRepository(private val context: Context) : SyncableRepository {
    private val TAG = "LandPrepRepository"
        ...
        try {
            ...
            if (isOnline) {
                try {
                    ...
                    context?.let { ctx ->
                        RestClient.getApiService(ctx).planLandPreparation(requestModel)
                        ...
                        RestClient.getApiService(ctx).planLandPreparation(requestModel)
                            .enqueue(object : Callback<LandPreparationModel> {
                                override fun onResponse(
                                    ...
                                    if (response.isSuccessful) {
                                        CoroutineScope(Dispatchers.IO).launch {
                                            landPrepDao.updateSyncStatus(landPrepWithDetails.landPrep.id, true)  // Changed here too
                                            successCount++
                                            Log.d(TAG, "Successfully synced land preparation ID: ${landPrepWithDetails.landPrep.id}")  // And here
                                        }
                                    } else {
                                        failureCount++
                                        Log.e(TAG, "Failed to sync land preparation ID: ${landPrepWithDetails.landPrep.id}")  // And here
                                    }
                                }
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            return@withContext suspendCancellableCoroutine { continuation ->
                context?.let { ctx ->
                    RestClient.getApiService(ctx).getLandPreparationPlans()
    ...
    private fun convertModelToEntities(model: LandPreparationModel): Quadruple<
        ...
        val coverCrop = model.coverCrop?.let {
            CoverCropEntity(
                landPrepId = 0, // Will be set after land prep is inserted
                coverCrop = it.coverCrop,
                dateOfEstablishment = it.dateOfEstablishment,
                unit = it.unit,
                unitCost = it.unitCost,
