...
class HarvestManagementRepository(private val context: Context) : SyncableRepository {
    private val TAG = "HarvestManagementRepository"
        ...
        try {
            ...
            if (isOnline) {
                try {
                    ...
                    context?.let { ctx ->
                        RestClient.getApiService(ctx).harvestingManagement(planHarvestingModel)
                            ...
                            .enqueue(object : Callback<PlanHarvestingModel> {
                                override fun onResponse(
                                    ...
                                    if (response.isSuccessful) {
                                        response.body()?.let {
                                            CoroutineScope(Dispatchers.IO).launch {
                                                harvestPlanningDao.markHarvestPlanningAsSynced(plan.id)
                                                buyers.forEach { buyer ->
                                                    harvestPlanningDao.markBuyerAsSynced(buyer.id)
                                                }
                                                successCount++
    ...
    override suspend fun syncFromServer(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            return@withContext suspendCancellableCoroutine { continuation ->
                context?.let { ctx ->
                    RestClient.getApiService(ctx).getHarvestingManagement()
                                    ...
                                    try {
                                        ...
                                        CoroutineScope(Dispatchers.IO).launch {
                                            ...
                                            serverPlans?.forEach { serverPlan ->
                                                ...
                                                val localPlanId = harvestPlanningDao.insertHarvestPlanning(localPlan)
                                                val buyers = createLocalBuyersFromServer(serverPlan, localPlanId)
                                                harvestPlanningDao.insertBuyers(buyers)
                                                savedCount++
                                            }
    ...
    private fun createPlanHarvestingModel(
        ...
        return PlanHarvestingModel(
            ...
            comments = harvestPlanning.comments ?: "",
            season_planning_id = harvestPlanning.seasonPlanningId,
            price_per_unit = harvestPlanning.pricePerUnit,  // Add this line
    ...
    private fun createLocalBuyersFromServer(serverPlan: PlanHarvestingModel, harvestPlanningId: Long): List<HarvestPlanningBuyer> {
        return serverPlan.buyers.map { buyerModel ->
            HarvestPlanningBuyer(
                ...
                harvestPlanningId = harvestPlanningId,
                isSynced = true
            )
        }
    }
